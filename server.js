...
const getRealClientIP = (req) => {
  // Check X-Forwarded-For header first (this is what Cloudflare uses)
  const forwardedFor = req.headers['x-forwarded-for'];
  if (forwardedFor) {
    // Get the first IP in the list (the real client IP)
    const ips = forwardedFor.split(',');
    // Return the first non-cloudflare IP
    for (let ip of ips) {
      ip = ip.trim();
      // Skip Cloudflare IP ranges
      if (!ip.startsWith('172.') && !ip.startsWith('173.') && !ip.startsWith('104.') && !ip.startsWith('108.') && !ip.startsWith('162.') && !ip.startsWith('192.') && !ip.startsWith('198.') && !ip.startsWith('203.')) {
        return ip;
      }
    }
    return ips[0].trim();
...
// Get gas fee for transaction (estimated based on network)
const getGasFee = async (asset, amount) => {
  try {
    // Default gas fees based on asset (in USD)
    const gasFeeMap = {
      'BTC': 2.50,
      'ETH': 3.00,
      'USDT': 2.00,
    ...
    let fee = gasFeeMap[asset.toUpperCase()] || 2.00;
    
    // If amount is provided, adjust fee based on percentage (0.5% of amount, minimum fee)
    if (amount) {
      const percentageFee = amount * 0.005;
      fee = Math.max(fee, Math.min(percentageFee, 50)); // Max fee $50
    }
    
    return fee;
  } catch (err) {
    console.error('Error getting gas fee:', err);
    return 2.00; // Default fallback fee
  }
};
...
const getUserDeviceInfo = async (req) => {
  try {
    // Enhanced IP detection with multiple header checks - bypass Cloudflare
  ...
  try {
    ...
    const getExchangeRate = async (asset, fiat = 'usd') => {
      try {
        ...
        const response = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${assetId}&vs_currencies=${fiat}`);
        const rate = await response.json();
        return rate[assetId]?.[fiat] || 0;
      } catch (error) {
    ...
    // Helper function to convert crypto to fiat with commas in amount
    ...
    // Helper function to format amount with commas and proper decimals
    ...
    // Helper function to get crypto logo URL (for transaction emails)
    const getCryptoLogoUrl = (asset) => {
      return getCryptoLogo(asset);
    };

    const templates = {
      // WELCOME EMAIL - Blue primary with golden accents, no boxes/blocks
      welcome: {
      ...
      // DEPOSIT RECEIVED - Transaction email with crypto logo, gas fee, real-time rates
      deposit_received: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .header { background-color: #1e3a8a; padding: 32px 40px; text-align: center; }
                ...
                .crypto-header { display: flex; align-items: center; gap: 16px; margin: 24px 0; padding: 16px; background: #f0fdf4; }
                ...
                .transaction-details { background: #f9fafb; padding: 24px; margin: 24px 0; }
                ...
                .address-value { font-family: monospace; background: #ffffff; padding: 4px 8px; font-size: 12px; border: 1px solid #e5e7eb; }
                ...
                .gold-accent { border-bottom: 2px solid #fbbf24; }
                @media only screen and (max-width: 600px) {
                  body { padding: 10px; }
                  .header { padding: 20px; }
                  .content { padding: 20px; }
                  ...
                  <div class="content">
                      ...
                      <div class="crypto-header">
                          <div class="crypto-icon">
                              <img src="${getCryptoLogoUrl(data.asset)}" alt="${data.asset || 'BTC'}" style="width: 32px; height: 32px;">
                          </div>
                          <div>
      ...
      // WITHDRAWAL REQUEST - Transaction email with crypto logo, gas fee, real-time rates
      withdrawal_request: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .crypto-header { display: flex; align-items: center; gap: 16px; margin: 24px 0; padding: 16px; background: #fef2f2; }
                .crypto-icon { width: 48px; height: 48px; border-radius: 50%; background: #ffffff; display: flex; align-items: center; justify-content: center; }
                .crypto-icon img { width: 32px; height: 32px; }
                .crypto-name { font-size: 20px; font-weight: 700; color: #1f2937; }
                .crypto-network { font-size: 12px; color: #6b7280; margin-top: 4px; }
                ...
                .detail-value { color: #1f2937; font-weight: 500; font-size: 14px; word-break: break-all; }
                .address-value { font-family: monospace; background: #ffffff; padding: 4px 8px; font-size: 12px; border: 1px solid #e5e7eb; }
                .processing-info { background: #fef3c7; padding: 16px; margin: 20px 0; }
      ...
      // WITHDRAWAL APPROVED - Transaction email with crypto logo and gas fee
      withdrawal_approved: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .crypto-icon img { width: 32px; height: 32px; }
                .crypto-name { font-size: 20px; font-weight: 700; color: #1f2937; }
                .approved-amount { font-size: 36px; font-weight: 700; color: #22c55e; margin: 12px 0; text-align: center; }
                .transaction-details { background: #f9fafb; padding: 24px; margin: 24px 0; }
                .detail-row { display: flex; justify-content: space-between; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb; }
                .detail-row:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
                .detail-label { color: #6b7280; font-size: 14px; font-weight: 500; }
                  ...
                  <div class="content">
                      ...
                      <div class="transaction-details">
                          ...
                          <div class="detail-row">
                              ...
                              <span class="detail-value address-value">${data.txid || data.transactionHash || data.transactionId || 'Broadcasted to Network'}</span>
                          </div>
                          <div class="detail-row">
      ...
      // INVESTMENT CREATED - No boxes/blocks, smooth page
      investment_created: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .investment-details { background: #f9fafb; padding: 24px; margin: 24px 0; }
                .detail-row { display: flex; justify-content: space-between; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb; }
                .detail-row:last-child { border-bottom: none; margin-bottom: 0; padding-bottom: 0; }
                .detail-label { color: #6b7280; font-size: 14px; font-weight: 500; }
                .detail-value { color: #1f2937; font-weight: 600; font-size: 16px; }
                .cta-button { background-color: #1e3a8a; color: #ffffff; padding: 12px 28px; text-decoration: none; display: inline-block; font-weight: 600; font-size: 15px; margin: 16px 0; border: 1px solid #fbbf24; }
                .footer { padding: 24px 40px; background-color: #f9fafb; text-align: center; }
                .footer-text { color: #6b7280; font-size: 12px; line-height: 1.5; }
                @media only screen and (max-width: 600px) {
                  body { padding: 10px; }
      ...
      // KYC APPROVED - No emojis, professional styling
      kyc_approved: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .success-box { background: #f0fdf4; padding: 24px; text-align: center; margin: 24px 0; }
                .success-text { font-size: 20px; font-weight: 600; color: #166534; }
                .benefits-list { margin: 24px 0; }
                .benefit-item { display: flex; align-items: center; margin-bottom: 12px; color: #4b5563; }
                .benefit-marker { color: #22c55e; margin-right: 12px; font-weight: bold; font-size: 18px; }
                .cta-button { background-color: #1e3a8a; color: #ffffff; padding: 12px 28px; text-decoration: none; display: inline-block; font-weight: 600; font-size: 15px; margin: 16px 0; border: 1px solid #fbbf24; }
                .footer { padding: 24px 40px; background-color: #f9fafb; text-align: center; }
                .footer-text { color: #6b7280; font-size: 12px; line-height: 1.5; }
                @media only screen and (max-width: 600px) {
              ...
              <div class="email-wrapper">
                  ...
                  <div class="content">
                      ...
                      <div class="benefits-list">
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Increased withdrawal limits</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Access to all investment plans</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Priority customer support</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Faster transaction processing</div>
                      </div>
      ...
      // KYC REJECTED - Professional styling
      kyc_rejected: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .rejected-box { background: #fef2f2; padding: 24px; text-align: center; margin: 24px 0; }
                .rejected-text { font-size: 20px; font-weight: 600; color: #991b1b; }
                .reason-box { background: #f9fafb; padding: 20px; margin: 24px 0; }
              ...
              <div class="email-wrapper">
                  ...
                  <div class="content">
                      ...
                      <div class="rejected-box">
                          <div class="rejected-text">Verification Issue</div>
                      </div>
      ...
      // LOGIN SUCCESS - Professional notification
      login_success: {
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .login-info { background: #f9fafb; padding: 20px; margin: 24px 0; }
                ...
                .security-note { background: #fef2f2; padding: 16px; margin: 24px 0; }
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .reset-button { background-color: #1e3a8a; color: #ffffff; padding: 14px 32px; text-decoration: none; display: inline-block; font-weight: 600; font-size: 16px; margin: 20px 0; border: 1px solid #fbbf24; }
                .security-note { background: #fef2f2; padding: 16px; margin: 24px 0; }
                .security-note p { color: #991b1b; font-size: 14px; }
                .footer { padding: 24px 40px; background-color: #f9fafb; text-align: center; }
                .footer-text { color: #6b7280; font-size: 12px; line-height: 1.5; }
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .feature-card { background: #f9fafb; padding: 20px; text-align: center; }
                .feature-title { font-weight: 600; color: #1e3a8a; margin-bottom: 8px; font-size: 16px; }
                .feature-desc { color: #6b7280; font-size: 13px; line-height: 1.4; }
                .benefits-list { margin: 24px 0; }
                .benefit-item { display: flex; align-items: center; margin-bottom: 12px; color: #4b5563; }
                .benefit-marker { color: #22c55e; margin-right: 12px; font-weight: bold; font-size: 18px; }
                .cta-button { background-color: #1e3a8a; color: #ffffff; padding: 14px 32px; text-decoration: none; display: inline-block; font-weight: 600; font-size: 16px; margin: 20px 0; border: 1px solid #fbbf24; }
                .security-note { background: #fef3c7; padding: 16px; margin: 24px 0; }
                .footer { padding: 24px 40px; background-color: #f9fafb; text-align: center; }
                .footer-text { color: #6b7280; font-size: 12px; line-height: 1.5; }
                .support-link { color: #2563eb; text-decoration: none; }
                @media only screen and (max-width: 600px) {
          ...
          <body style="background-color: #f4f4f5; padding: 20px;">
              <div class="email-wrapper">
                  ...
                  <div class="content">
                      ...
                      <div class="benefits-list">
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Access enterprise-grade Bitcoin mining infrastructure</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Invest in optimized mining plans with competitive returns</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Monitor your mining operations in real-time</div>
                          <div class="benefit-item"><span class="benefit-marker">✓</span> Earn referral bonuses by expanding our mining community</div>
                      </div>
          ...
          <head>
              ...
              <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .greeting { font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 16px; }
                .message { color: #4b5563; line-height: 1.6; margin-bottom: 24px; font-size: 16px; }
                .otp-code { background: #f9fafb; padding: 24px; font-size: 48px; font-weight: 700; text-align: center; letter-spacing: 12px; margin: 24px 0; color: #2563eb; font-family: 'Courier New', monospace; }
                .security-note { background: #fef2f2; padding: 16px; margin: 24px 0; }
                .security-note p { color: #991b1b; font-size: 14px; }
                .footer { padding: 24px 40px; background-color: #f9fafb; text-align: center; }
                .footer-text { color: #6b7280; font-size: 12px; line-height: 1.5; }
...
// Admin Approve Deposit Endpoint - ENHANCED WITH EMAIL, REAL-TIME CRYPTO PRICE, AND GAS FEE
app.post('/api/admin/deposits/:id/approve', adminProtect, [
  ...
  try {
    ...
    if (deposit.method && deposit.method !== 'BANK' && deposit.method !== 'CARD') {
      ...
      // Get gas fee for the transaction
      fee = await getGasFee(deposit.method, deposit.amount);
      feeUsd = fee;
      netAmount = deposit.amount - fee;
    }
    
    // Update user balance (net amount after gas fee)
    ...
    deposit.fee = fee;
    deposit.netAmount = netAmount;
    await deposit.save();

    // ✅ ENHANCED: Send deposit approved email with crypto logo, real-time rates, and gas fee
    try {
      await sendAutomatedEmail(user, 'deposit_received', {
        ...
        feeUsd: feeUsd,
        netAmount: netAmount,
        txid: deposit.details?.txid || deposit.reference,
        fromAddress: deposit.details?.fromAddress,
        toAddress: deposit.details?.toAddress,
        timestamp: deposit.createdAt,
        confirmations: 6,
        network: deposit.method === 'USDT' ? 'ERC-20' : deposit.method === 'BTC' ? 'Bitcoin' : 'Mainnet'
      });
...
// Admin Approve Withdrawal Endpoint - ENHANCED WITH EMAIL, REAL-TIME CRYPTO PRICE, AND GAS FEE
  ...
  try {
    ...
    if (withdrawal.asset && withdrawal.asset !== 'USD') {
      ...
      if (cryptoPrice) {
        ...
        feeUsd = fee * cryptoPrice;
        netAmount = (withdrawal.assetAmount || withdrawal.amount) - fee;
      }
    }
    ...
    // ✅ ENHANCED: Send withdrawal approved email with crypto logo, real-time rates, and gas fee
    try {
      await sendAutomatedEmail(withdrawal.user, 'withdrawal_approved', {
        ...
        network: withdrawal.asset === 'USDT' ? 'ERC-20' : withdrawal.asset === 'BTC' ? 'Bitcoin' : 'Mainnet'
      });
