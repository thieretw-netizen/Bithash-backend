...
const ensureUploadDirectories = () => {
  ...
  const identityPath = process.env.KYC_IDENTITY_PATH || path.join(uploadPath, 'identity');
  const addressPath = process.env.KYC_ADDRESS_PATH || path.join(uploadPath, 'address');
  const facialPath = process.env.KYC_FACIAL_PATH || path.join(uploadPath, 'facial');
  
  const dirs = [identityPath, addressPath, facialPath, tempPath];
...
const getUploadDestination = (fieldname) => {
  ...
  if (fieldname.includes('identity')) {
    return identityPath;
  } else if (fieldname.includes('address')) {
    return addressPath;
  } else if (fieldname.includes('facial')) {
    return facialPath;
  }
  return tempPath;
};
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = getUploadDestination(file.fieldname);
    cb(null, uploadPath);
  },
...
const fileFilter = (req, file, cb) => {
  const allowedMimes = allowedMimeTypes.reduce((acc, mime) => {
    acc[mime.trim()] = true;
    return acc;
  }, {});
...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    // If isSignup is true (from signup page) and user does NOT exist, create new user
    if (isSignup === true && !user) {
      if (process.env.NODE_ENV !== 'production') console.log('Creating new user via Google signup');
    ...
    // If isSignup is false (from login page) and user does NOT exist, return error
    else if (isSignup === false && !user) {
      if (process.env.NODE_ENV !== 'production') console.log('Login attempt with non-existent Google account');
      return res.status(404).json({
        status: 'fail',
    ...
    // Check if user is active (only if user exists)
    ...
    try {
      ...
      await UserLog.create({
        ...
        metadata: {
          ...
          loginMethod: 'google',
          otpSent: true,
          isNewUser: isSignup === true && !user?.googleId
        }
      });
