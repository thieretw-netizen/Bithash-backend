...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    if (!user) {
      // Check if this is from login page (isSignup: false) or signup page (isSignup: true)
      const { isSignup } = req.body;
      
      if (isSignup === false) {
        // User trying to login with Google but doesn't exist
        return res.status(404).json({
          status: 'fail',
...
// Function to start real-time price updates
