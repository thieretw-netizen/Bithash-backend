...
// =============================================
// GET /api/admin/crypto/assets - Fetch all crypto assets with real-time prices and logos
// =============================================
app.get('/api/admin/crypto/assets', adminProtect, async (req, res) => {
  try {
    ...
    // Aggregate crypto holdings across ALL users from MAIN and MATURED wallets
    const totalHoldings = new Map();
    ...
    const assetNames = {
      ...
      'near': 'NEAR Protocol', 'uni': 'Uniswap', 'bch': 'Bitcoin Cash',
      'xlm': 'Stellar', 'atom': 'Cosmos', 'xmr': 'Monero', 'flow': 'Flow',
      'vet': 'VeChain', 'fil': 'Filecoin', 'theta': 'Theta Network',
      'hbar': 'Hedera', 'ftm': 'Fantom', 'xtz': 'Tezos'
    };
    ...
    for (const user of users) {
      ...
      if (user.balances.main) {
        ...
        for (const [asset, balance] of entries) {
          if (balance > 0 && asset !== 'usd') {
            const current = totalHoldings.get(asset) || { totalAmount: 0 };
            current.totalAmount += balance;
            totalHoldings.set(asset, current);
          }
        }
      }
    ...
    const assets = Array.from(totalHoldings.keys());
    const prices = {};
    
    for (const asset of assets) {
      try {
        const price = await getCryptoPrice(asset.toUpperCase());
        prices[asset] = (price && price > 0) ? price : 1;
      } catch (err) {
        const fallbackPrices = {
          'btc': 50000, 'eth': 3000, 'usdt': 1, 'bnb': 300, 'sol': 100,
          'usdc': 1, 'xrp': 0.5, 'doge': 0.08, 'ada': 0.3, 'shib': 0.00001,
          'avax': 20, 'dot': 5, 'trx': 0.08, 'link': 15, 'matic': 0.8,
          'ltc': 70, 'near': 2, 'uni': 5, 'bch': 200, 'xlm': 0.1,
          'atom': 8, 'xmr': 150, 'flow': 0.5, 'vet': 0.02, 'fil': 3,
          'theta': 1, 'hbar': 0.05, 'ftm': 0.3, 'xtz': 0.8
        };
        prices[asset] = fallbackPrices[asset] || 1;
      }
    }
    ...
    for (const [asset, holdings] of totalHoldings) {
      const price = prices[asset] || 0;
      const totalValueUSD = holdings.totalAmount * price;
      totalPlatformValue += totalValueUSD;
      
      cryptoAssets.push({
        symbol: asset.toUpperCase(),
        name: assetNames[asset] || asset.toUpperCase(),
        logoUrl: assetLogos[asset] || `https://cryptologos.cc/logos/${asset}-${asset}-logo.png`,
    ...
    cryptoAssets.forEach(asset => {
      asset.percentageOfTotal = totalPlatformValue > 0 
        ? (asset.totalValueUSD / totalPlatformValue) * 100 
        : 0;
    });
    
    cryptoAssets.sort((a, b) => b.totalValueUSD - a.totalValueUSD);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        lastUpdated: new Date(),
        assetCount: cryptoAssets.length
      }
    });
...
// =============================================
// GET /api/admin/transactions/volume - Get transaction volume for chart (last 7 days)
// =============================================
app.get('/api/admin/transactions/volume', adminProtect, async (req, res) => {
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      dailyVolume.set(dateStr, { volume: 0, count: 0, deposits: 0, withdrawals: 0, investments: 0, label: formattedLabel });
    ...
    for (const tx of transactions) {
      ...
      if (dailyVolume.has(dateStr)) {
        const dayData = dailyVolume.get(dateStr);
        dayData.volume += tx.amount || 0;
        dayData.count += 1;
        
        if (tx.type === 'deposit') dayData.deposits += tx.amount || 0;
        else if (tx.type === 'withdrawal') dayData.withdrawals += tx.amount || 0;
        else if (tx.type === 'investment') dayData.investments += tx.amount || 0;
        
        dailyVolume.set(dateStr, dayData);
      }
    }
    ...
    for (let i = 0; i < days; i++) {
      ...
      date.setDate(startDate.getDate() + i);
      const dateStr = date.toISOString().split('T')[0];
      const dayData = dailyVolume.get(dateStr) || { volume: 0, count: 0 };
      values.push(dayData.volume);
    }
    ...
    typeStats.forEach(stat => {
      typeBreakdown[stat._id] = {
        volume: stat.totalVolume,
        count: stat.count
      };
    });
    
    const dailyBreakdown = Array.from(dailyVolume.entries()).map(([date, data]) => ({
      date,
      label: data.label,
      volume: data.volume,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        averageDailyVolume: values.length > 0 ? values.reduce((a, b) => a + b, 0) / values.length : 0
      }
    });
...
// =============================================
// GET /api/admin/statements - Get financial statements history with pagination
// =============================================
app.get('/api/admin/statements', adminProtect, async (req, res) => {
  try {
    ...
    const formattedStatements = statements.map(statement => ({
      ...
      openingBalanceUSD: statement.openingBalances?.totalUSD || 0,
      closingBalanceUSD: statement.closingBalances?.totalUSD || 0,
      netChangeUSD: statement.netChangeUSD || 0,
...
app.post('/api/admin/statements/generate', adminProtect, async (req, res) => {
  try {
    ...
    for (const user of users) {
      try {
        const statement = await generateFinancialStatement(user._id, period, startDate, endDate);
        
        if (statement) {
          const pdfBuffer = await generateProfessionalPDFStatement(statement, user, period);
          
          await sendFinancialStatementEmailWithPDF(user, statement, period, pdfBuffer);
    ...
    res.status(200).json({
      status: 'success',
      message: `Statements processed: ${successCount} sent, ${failCount} failed`,
      data: {
        total: users.length,
...
// =============================================
// Helper: Generate Financial Statement
// =============================================
async function generateFinancialStatement(userId, period, startDate, endDate) {
  try {
    ...
    for (const tx of transactions) {
      if (tx.type === 'deposit') totalDepositsUSD += tx.amount || 0;
      if (tx.type === 'withdrawal') totalWithdrawalsUSD += tx.amount || 0;
      if (tx.fee) totalFeesUSD += tx.fee;
      if (tx.type === 'interest') totalInvestmentProfitUSD += tx.amount || 0;
      if (tx.type === 'referral') totalReferralEarningsUSD += tx.amount || 0;
      if (tx.type === 'sell' && tx.sellDetails?.profitLoss > 0) {
        totalTradingProfitUSD += tx.sellDetails.profitLoss;
      }
      if (tx.type === 'sell' && tx.sellDetails?.profitLoss < 0) {
        totalTradingLossUSD += Math.abs(tx.sellDetails.profitLoss);
      }
    ...
    const activeInvestments = investments
      .filter(inv => inv.status === 'active' && inv.endDate > new Date())
      .map(inv => ({
    ...
    const maturedInvestments = investments
    ...
    const totalLossUSD = totalFeesUSD + totalTradingLossUSD;
    ...
    const statement = await FinancialStatement.create({
      ...
      transactions: {
        ...
        summary: {
          ...
          totalTransfersUSD: 0,
          count: {
            deposits: transactionList.filter(t => t.type === 'deposit').length,
            withdrawals: transactionList.filter(t => t.type === 'withdrawal').length,
            transfers: transactionList.filter(t => t.type === 'transfer').length
          }
        }
      },
      investments: {
        ...
        started: [],
        summary: {
          totalPrincipalInvestedUSD: activeInvestments.reduce((sum, inv) => sum + inv.principalUSD, 0),
          totalReturnsEarnedUSD: maturedInvestments.reduce((sum, inv) => sum + inv.profitUSD, 0),
          totalProfitUSD: maturedInvestments.reduce((sum, inv) => sum + inv.profitUSD, 0),
          totalActiveInvestmentsCount: activeInvestments.length,
          totalActivePrincipalUSD: activeInvestments.reduce((sum, inv) => sum + inv.principalUSD, 0)
        }
      },
      trading: {
        buys: [],
      ...
      referrals: {
        ...
        summary: {
          ...
          directReferralEarningsUSD: totalReferralEarningsUSD,
...
// =============================================
// Helper: Generate Professional PDF Statement
// =============================================
async function generateProfessionalPDFStatement(statement, user, period) {
  ...
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ 
        ...
        info: {
          Title: `Financial Statement - ${period.toUpperCase()} - ${user.firstName} ${user.lastName}`,
      ...
      doc.fontSize(24)
        .font('Helvetica-Bold')
        .fillColor('#1a1a2e')
        .text('₿itHash Capital', 120, 50);
      
      doc.fontSize(10)
        .font('Helvetica')
        .fillColor('#666666')
        .text('Institutional Bitcoin Mining', 120, 78);
      ...
      const startDateStr = new Date(statement.period.startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      const endDateStr = new Date(statement.period.endDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      
      doc.fontSize(18)
        .font('Helvetica-Bold')
        .fillColor('#1a1a2e')
        .text(`${periodText} Financial Statement`, 50, 130);
      
      doc.fontSize(10)
        .font('Helvetica')
        .fillColor('#666666')
        .text(`Period: ${startDateStr} - ${endDateStr}`, 50, 158);
      
      doc.fontSize(10)
        .font('Helvetica')
        .fillColor('#666666')
        .text(`Statement Reference: ${statement.reference}`, 50, 178);
      
      doc.fontSize(10)
        .font('Helvetica')
        .fillColor('#666666')
        .text(`Generated: ${new Date().toLocaleString()}`, 50, 198);
      ...
      doc.fontSize(12)
        .font('Helvetica-Bold')
        .fillColor('#1a1a2e')
        .text('Account Holder Information', 50, 240);
      
      doc.fontSize(10)
        .font('Helvetica')
        .fillColor('#333333')
        .text(`Name: ${user.firstName} ${user.lastName}`, 50, 265);
      
      doc.text(`Email: ${user.email}`, 50, 285);
      
      doc.moveTo(300, 240)
        .font('Helvetica-Bold')
        .text('Account Summary', 300, 240);
      
      doc.font('Helvetica')
        .fontSize(10)
        .fillColor('#333333')
        .text(`Opening Balance: $${statement.openingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, 265);
      
      doc.text(`Closing Balance: $${statement.closingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, 285);
      
      const netChangeColor = statement.netChangeUSD >= 0 ? '#10b981' : '#ef4444';
      doc.fillColor(netChangeColor)
        .text(`Net Change: ${statement.netChangeUSD >= 0 ? '+' : ''}$${statement.netChangeUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, 305);
      ...
      // Main Wallet
      doc.fillColor('#333333')
        .text('Main Wallet (Crypto Holdings):', 50, walletY);
      doc.fillColor('#10b981')
        .text(`$${statement.closingBalances.mainWalletUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 400, walletY, { align: 'right', width: 150 });
      
      // Active Wallet (Mining Contracts)
      doc.fillColor('#333333')
        .text('Active Wallet (Mining Contracts):', 50, walletY + 25);
      doc.fillColor('#f59e0b')
        .text(`$${statement.closingBalances.activeWalletUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 400, walletY + 25, { align: 'right', width: 150 });
      
      // Matured Wallet
      doc.fillColor('#333333')
        .text('Matured Wallet (Completed Investments):', 50, walletY + 50);
      doc.fillColor('#8b5cf6')
        .text(`$${statement.closingBalances.maturedWalletUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 400, walletY + 50, { align: 'right', width: 150 });
      
      doc.fillColor('#333333');
      
      // Crypto Holdings Details
      if (statement.closingBalances.cryptoDetails && statement.closingBalances.cryptoDetails.length > 0) {
        doc.moveTo(50, walletY + 85)
          .lineTo(550, walletY + 85)
          .stroke();
        
        doc.fontSize(11)
          .font('Helvetica-Bold')
          .text('Crypto Asset Holdings', 50, walletY + 105);
        
        let cryptoY = walletY + 130;
        doc.fontSize(9)
          .font('Helvetica-Bold')
          .fillColor('#666666')
          .text('Asset', 50, cryptoY)
          .text('Amount', 200, cryptoY)
          .text('Price (USD)', 300, cryptoY)
          .text('Value (USD)', 400, cryptoY);
        
        cryptoY += 20;
        doc.fillColor('#333333')
          .font('Helvetica');
        
        for (const crypto of statement.closingBalances.cryptoDetails.slice(0, 10)) {
          if (cryptoY > 700) {
            doc.addPage();
            cryptoY = 50;
          }
          doc.text(crypto.asset.toUpperCase(), 50, cryptoY)
            .text(crypto.amount.toFixed(8), 200, cryptoY)
            .text(`$${(crypto.usdValue / crypto.amount).toFixed(2)}`, 300, cryptoY)
            .text(`$${crypto.usdValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 400, cryptoY);
          cryptoY += 20;
        }
      }
      ...
      doc.fontSize(11)
        .font('Helvetica-Bold')
        .fillColor('#10b981')
        .text(`Total Inflow: $${statement.summary.totalInflowUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 50, summaryY);
      
      doc.fillColor('#ef4444')
        .text(`Total Outflow: $${statement.summary.totalOutflowUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 50, summaryY + 25);
      
      doc.fillColor('#3b82f6')
        .text(`Net Cash Flow: $${statement.summary.netCashFlowUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 50, summaryY + 50);
      
      doc.fillColor('#10b981')
        .text(`Total Profit: $${statement.summary.totalProfitUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, summaryY);
      
      doc.fillColor('#ef4444')
        .text(`Total Loss: $${statement.summary.totalLossUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, summaryY + 25);
      
      doc.fillColor('#1a1a2e')
        .font('Helvetica-Bold')
        .text(`Net Profit: $${statement.summary.netProfitUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, summaryY + 50);
      
      doc.fillColor('#8b5cf6')
        .text(`ROI: ${statement.summary.roiPercentage.toFixed(2)}%`, 300, summaryY + 75);
      
      doc.fillColor('#333333');
      
      doc.moveTo(50, summaryY + 110)
        .lineTo(550, summaryY + 110)
        .stroke();
      
      // Recent Transactions Table
      doc.fontSize(12)
        .font('Helvetica-Bold')
        .text('Recent Transactions', 50, summaryY + 130);
      
      let txY = summaryY + 155;
      doc.fontSize(8)
        .font('Helvetica-Bold')
        .fillColor('#666666')
        .text('Date', 50, txY)
      ...
      for (const tx of recentTransactions) {
        ...
        const txAmount = tx.type === 'withdrawal' ? `-$${tx.amountUSD.toFixed(2)}` : `+$${tx.amountUSD.toFixed(2)}`;
        const txColor = tx.type === 'withdrawal' ? '#ef4444' : '#10b981';
        
        doc.fillColor('#666666')
          .text(new Date(tx.createdAt).toLocaleDateString(), 50, txY)
          .text(tx.type, 130, txY)
          .text(tx.asset || 'USD', 200, txY);
        
        doc.fillColor(txColor)
          .text(txAmount, 280, txY);
        
        doc.fillColor('#666666')
          .text(tx.status, 370, txY)
          .text(tx.reference?.slice(-8) || 'N/A', 440, txY);
      ...
      if (statement.investments.matured.length > 0) {
        ...
        doc.fontSize(14)
          .font('Helvetica-Bold')
          .fillColor('#1a1a2e')
          .text('Investment Performance', 50, 50);
        ...
        doc.fontSize(9)
          ...
          .fillColor('#666666')
          .text('Plan', 50, invY)
          .text('Initial', 180, invY)
          .text('Return', 280, invY)
          .text('Profit', 380, invY)
          .text('ROI', 470, invY);
        ...
        for (const inv of statement.investments.matured) {
          ...
          doc.text(inv.planName, 50, invY)
            .text(`$${inv.initialAmountUSD.toLocaleString()}`, 180, invY)
            .text(`$${inv.returnAmountUSD.toLocaleString()}`, 280, invY);
          
          const profitColor = inv.profitUSD >= 0 ? '#10b981' : '#ef4444';
          doc.fillColor(profitColor)
            .text(`${inv.profitUSD >= 0 ? '+' : ''}$${inv.profitUSD.toLocaleString()}`, 380, invY)
            .text(`${inv.profitPercentage}%`, 470, invY);
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        doc.fontSize(8)
          .fillColor('#999999')
          .text(
            `BitHash Capital - Financial Statement - Page ${i + 1} of ${totalPages}`,
            50,
            doc.page.height - 50,
        ...
        doc.fontSize(7)
          .text(
            'This is an automatically generated statement. For any discrepancies, please contact support@bithashcapital.live',
            50,
            doc.page.height - 35,
            { align: 'center', width: doc.page.width - 100 }
          );
...
// =============================================
async function sendFinancialStatementEmailWithPDF(user, statement, period, pdfBuffer) {
  try {
    const periodText = period === 'weekly' ? 'Weekly' : 'Monthly';
    ...
    const cryptoLogoUrl = 'https://media.bithashcapital.live/ChatGPT%20Image%20Mar%2029%2C%202026%2C%2004_52_02%20PM.png';
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <style>
          body { font-family: 'Inter', 'Segoe UI', Arial, sans-serif; margin: 0; padding: 0; background-color: #f5f5f5; }
          .container { max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 35px -10px rgba(0,0,0,0.1); }
          .header { background: linear-gradient(135deg, #0B0E11 0%, #11151C 100%); padding: 30px 30px 20px 30px; text-align: center; }
          ...
          .header p { color: #B7BDC6; font-size: 14px; margin: 10px 0 0 0; }
          .content { padding: 30px; }
          .greeting { font-size: 16px; font-weight: 600; color: #111827; margin-bottom: 20px; }
          .period-badge { background: #F3F4F6; padding: 8px 16px; border-radius: 40px; display: inline-block; font-size: 13px; font-weight: 500; color: #4B5563; margin-bottom: 25px; }
          .stats-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin: 25px 0; }
          ...
          .profit-card { background: ${netProfit >= 0 ? '#ECFDF5' : '#FEF2F2'}; border: 1px solid ${netProfit >= 0 ? '#A7F3D0' : '#FEE2E2'}; }
          .profit-value { color: ${profitColor}; font-size: 28px; }
          .divider { height: 1px; background: #E5E7EB; margin: 25px 0; }
          .transaction-item { padding: 12px 0; border-bottom: 1px solid #F3F4F6; display: flex; justify-content: space-between; align-items: center; }
          .transaction-type { font-weight: 500; font-size: 14px; color: #374151; }
          .transaction-date { font-size: 11px; color: #9CA3AF; }
          .transaction-amount { font-weight: 600; font-size: 14px; }
          .amount-positive { color: #10B981; }
          .amount-negative { color: #EF4444; }
          .view-all-btn { display: block; text-align: center; margin-top: 20px; padding: 12px; background: #F7A600; color: #000000; text-decoration: none; border-radius: 999px; font-weight: 600; transition: all 0.3s ease; }
          .view-all-btn:hover { background: #E89500; transform: translateY(-2px); }
          .footer { background: #0B0E11; padding: 20px; text-align: center; border-top: 1px solid #1E2329; }
          .footer p { color: #6C7480; font-size: 12px; margin: 5px 0; }
          .footer a { color: #F7A600; text-decoration: none; }
          .attachment-note { background: #EFF6FF; padding: 15px; border-radius: 12px; margin: 20px 0; text-align: center; border: 1px solid #BFDBFE; }
          .attachment-note i { color: #3B82F6; margin-right: 8px; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <img src="${cryptoLogoUrl}" alt="BitHash Logo" class="logo">
            <h1>₿itHash Capital</h1>
            <p><i>Where Your Financial Goals Become Reality</i></p>
          </div>
          ...
          <div class="content">
            ...
            <div style="text-align: center;">
              <span class="period-badge">
                📊 ${periodText} Financial Statement • ${startDateStr} - ${endDateStr}
              </span>
            </div>
            ...
            <div class="stats-grid">
              <div class="stat-card">
                ...
                <div class="stat-value">$${statement.openingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div class="stat-card">
                <div class="stat-label">Closing Balance</div>
                <div class="stat-value">$${statement.closingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div class="stat-card profit-card">
                <div class="stat-label">Net ${periodText} Profit/Loss</div>
                <div class="stat-value profit-value">${profitSymbol}$${Math.abs(netProfit).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
              </div>
              <div class="stat-card">
                <div class="stat-label">ROI</div>
                <div class="stat-value ${statement.summary.roiPercentage >= 0 ? '' : 'profit-value'}">${statement.summary.roiPercentage.toFixed(2)}%</div>
              </div>
            </div>
            
            <div class="divider"></div>
            
            <h3 style="font-size: 16px; margin: 0 0 15px 0;">📋 Recent Activity</h3>
            
            ${statement.transactions.list.slice(0, 5).map(tx => `
              <div class="transaction-item">
                ...
                <div class="transaction-amount ${tx.type === 'withdrawal' ? 'amount-negative' : 'amount-positive'}">
                  ${tx.type === 'withdrawal' ? '-' : '+'}$${tx.amountUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </div>
              </div>
            `).join('')}
            
            ${statement.transactions.list.length > 5 ? `
              <a href="https://www.bithashcapital.live/dashboard" class="view-all-btn">
                View All Transactions →
              </a>
            ` : ''}
            
            <div class="attachment-note">
              <i>📎</i> <strong>PDF Attachment</strong><br>
              A detailed PDF version of your ${periodText.toLowerCase()} financial statement is attached to this email. 
              It includes complete transaction history, wallet breakdowns, and investment performance metrics.
            </div>
            
            <div class="divider"></div>
            
            <div style="background: #F5F5F5; border-radius: 12px; padding: 15px; margin-top: 20px;">
              <p style="margin: 0 0 10px 0; font-weight: 600; color: #111827;">📊 Key Metrics</p>
              <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 8px;">
                <span style="color: #6B7280;">Total Deposits:</span>
                <span style="font-weight: 500;">$${statement.transactions.summary.totalDepositsUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 8px;">
                <span style="color: #6B7280;">Total Withdrawals:</span>
                <span style="font-weight: 500;">$${statement.transactions.summary.totalWithdrawalsUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 8px;">
                <span style="color: #6B7280;">Investment Returns:</span>
                <span style="font-weight: 500; color: #10B981;">+$${statement.investments.summary.totalReturnsEarnedUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              <div style="display: flex; justify-content: space-between; font-size: 13px;">
                <span style="color: #6B7280;">Referral Earnings:</span>
                <span style="font-weight: 500; color: #10B981;">+$${statement.referrals.summary.totalReferralEarningsUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
            
            <p style="font-size: 12px; color: #6B7280; margin-top: 25px; text-align: center;">
              This is an automatically generated statement. For any discrepancies, please contact our support team.
            </p>
          </div>
    ...
    const subject = `${periodText} Financial Statement - ₿itHash Capital (${startDateStr} to ${endDateStr})`;
    ...
    await mailTransporter.sendMail(mailOptions);
    console.log(`📧 Financial statement email with PDF sent to ${user.email}`);
