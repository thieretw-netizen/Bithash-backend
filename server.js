...
// =============================================
// UNIVERSAL REQUEST LOGGER - Logs EVERY request to SystemLog in real-time
// =============================================

const universalRequestLogger = async (req, res, next) => {
  ...
  // Capture request headers (safe ones only)
  const safeHeaders = {
  ...
  try {
    ...
    else if (/iPad|Android|Touch/i.test(userAgent)) deviceInfo.type = 'tablet';
    else deviceInfo.type = 'desktop';
    
    if (/windows/i.test(userAgent)) deviceInfo.os = 'Windows';
    else if (/macintosh|mac os x/i.test(userAgent)) deviceInfo.os = 'MacOS';
    ...
    if (/edg/i.test(userAgent)) deviceInfo.browser = 'Edge';
    else if (/chrome/i.test(userAgent)) deviceInfo.browser = 'Chrome';
  ...
  res.end = function(chunk, encoding) {
    ...
    let entity = 'api';
    let entityId = null;
    let actionCategory = 'navigation';
    
    // Map URL patterns to actions
    const url = req.originalUrl || req.url;
    
    if (url.includes('/api/auth/login')) action = 'login';
    else if (url.includes('/api/auth/signup')) action = 'user_signup';
    else if (url.includes('/api/auth/logout')) action = 'logout';
    else if (url.includes('/api/investments') && req.method === 'POST') action = 'investment_created';
    else if (url.includes('/api/investments') && req.method === 'GET') action = 'investment_view';
    else if (url.includes('/api/withdrawals') && req.method === 'POST') action = 'withdrawal_create';
    else if (url.includes('/api/deposits') && req.method === 'POST') action = 'deposit_create';
    else if (url.includes('/api/convert') && req.method === 'POST') action = 'conversion_execute';
    else if (url.includes('/api/users/me') && req.method === 'GET') action = 'profile_view';
    else if (url.includes('/api/users/profile') && req.method === 'PUT') action = 'profile_update';
    else if (url.includes('/api/users/password') && req.method === 'PUT') action = 'password_change';
    else if (url.includes('/api/users/two-factor') && req.method === 'POST') action = '2fa_enable';
    else if (url.includes('/api/users/two-factor') && req.method === 'DELETE') action = '2fa_disable';
    else if (url.includes('/api/kyc')) action = 'kyc_submission';
    else if (url.includes('/api/referrals')) action = 'referral_view';
    else if (url.includes('/api/loans') && req.method === 'POST') action = 'loan_request';
    else if (url.includes('/api/loans') && req.method === 'GET') action = 'loan_view';
    else if (url.includes('/api/admin')) action = 'admin_action';
    
    // Determine entity
    if (url.includes('/users') || url.includes('/profile')) entity = 'user';
    else if (url.includes('/investments')) entity = 'investment';
    else if (url.includes('/withdrawals')) entity = 'withdrawal';
    else if (url.includes('/deposits')) entity = 'deposit';
    else if (url.includes('/convert')) entity = 'conversion';
    else if (url.includes('/kyc')) entity = 'kyc';
    else if (url.includes('/admin')) entity = 'admin';
    else if (url.includes('/auth')) entity = 'authentication';
    
    // Determine category
    if (url.includes('/auth')) actionCategory = 'authentication';
    else if (url.includes('/investments')) actionCategory = 'investment';
    else if (url.includes('/withdrawals') || url.includes('/deposits') || url.includes('/convert')) actionCategory = 'financial';
    else if (url.includes('/kyc')) actionCategory = 'verification';
    else if (url.includes('/profile') || url.includes('/users')) actionCategory = 'profile';
    else if (url.includes('/admin')) actionCategory = 'system';
    else actionCategory = 'navigation';
    
    // Get user info if authenticated
    let performedBy = null;
    let performedByModel = null;
    ...
    try {
      const ip = getRealClientIP(req);
      const ipinfoToken = process.env.IPINFO_TOKEN || 'b56ce6e91d732d';
      const response = await axios.get(`https://ipinfo.io/${ip}?token=${ipinfoToken}`, { timeout: 2000 });
      if (response.data) {
        ...
        if (response.data.loc && response.data.loc.includes(',')) {
          const coords = response.data.loc.split(',');
          latitude = parseFloat(coords[0]);
          longitude = parseFloat(coords[1]);
        }
      ...
      // Silent fail - location will remain Unknown
    }
    
    // Create system log entry ASYNC (don't await - don't block response)
    SystemLog.create({
      action: action,
      entity: entity,
      entityId: entityId,
      ...
      status: statusCode >= 200 && statusCode < 400 ? 'success' : statusCode >= 400 ? 'failed' : 'pending',
      riskLevel: statusCode >= 500 ? 'high' : statusCode >= 400 ? 'medium' : 'low',
      sessionId: req.session?.id,
      requestId: req.id || crypto.randomBytes(16).toString('hex'),
      metadata: {
    ...
    }).catch(err => console.error('SystemLog creation error:', err));
...
// Apply the universal logger to ALL routes (BEFORE your routes)
app.use(universalRequestLogger);
