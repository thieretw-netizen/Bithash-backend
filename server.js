...
app.get('/api/users/preferences', protect, async (req, res) => {
  try {
    // Check if user has preferences in their document
    if (req.user.preferences && req.user.preferences.displayAsset) {
      return res.status(200).json({
        status: 'success',
        displayAsset: req.user.preferences.displayAsset,
