...
redis.on('error', (err) => {
  if (process.env.NODE_ENV !== 'production') console.log('Redis error:', err);
});
...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    // Check if isSignup flag is false (from login page) and user doesn't exist
    if (isSignup === false && !user) {
      if (process.env.NODE_ENV !== 'production') console.log('Login attempted with Google for non-existent user');
      return res.status(404).json({
        status: 'fail',
