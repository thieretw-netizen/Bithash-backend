...
// =============================================
// TRADING ENDPOINTS - Order Book, Recent Trades, Buy/Sell Orders
// =============================================

// Import required models at the top if not already imported
// Make sure these models exist - add them if they don't

// Order Book Schema for storing generated orders
const OrderBookSchema = new mongoose.Schema({
  ...
  userId: { 
    ...
    ref: 'User',
    index: true,
    default: null // null for generated orders
  },
  isGenerated: { 
    type: Boolean, 
    default: false,
  ...
  metadata: {
    exchangeRate: Number,
    source: { type: String, enum: ['generated', 'user'], default: 'user' }
  }
}, { 
  timestamps: true,
  toJSON: { virtuals: true },
...
OrderBookSchema.virtual('remainingAmount').get(function() {
  return this.amount - this.filledAmount;
});

OrderBookSchema.virtual('isFullyFilled').get(function() {
  return this.filledAmount >= this.amount;
});

OrderBookSchema.index({ symbol: 1, type: 1, price: -1 });
OrderBookSchema.index({ symbol: 1, status: 1, createdAt: -1 });
OrderBookSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });

// Recent Trades Schema for user trades
...
async function fetchExchangeRate(symbol = 'btc') {
  try {
    ...
    // Fallback to cached rate from Redis
    try {
      const cachedRate = await redis.get(`exchange_rate:${symbol}`);
      if (cachedRate) {
    ...
    // Ultimate fallback - use last known rate from database
    const lastOrder = await OrderBook.findOne({ 
      symbol: symbol.toUpperCase() + 'USD',
      'metadata.exchangeRate': { $exists: true }
    }).sort({ createdAt: -1 });
    
    if (lastOrder && lastOrder.metadata.exchangeRate) {
      return lastOrder.metadata.exchangeRate;
    }
    
    // Default fallback
    return symbol === 'btc' ? 43000 : 3000; // Default BTC price
  }
}
...
async function generateRandomOrders() {
  ...
  const baseSymbol = selectedSymbol.replace('USD', '').toLowerCase();
  
  try {
    // Get current exchange rate
    const basePrice = await fetchExchangeRate(baseSymbol);
    
    // Store in Redis for real-time access
    await redis.setex(`exchange_rate:${baseSymbol}`, 60, basePrice.toString());
    ...
    for (let i = 0; i < numOrders; i++) {
      ...
      // Generate random amount between 0.001 and 5
      const amount = Math.round((Math.random() * 4.999 + 0.001) * 1000) / 1000;
      const total = price * amount;
      
      // Set expiration to 5-30 seconds from now
      const expiresIn = Math.floor(Math.random() * 25000) + 5000; // 5-30 seconds
    ...
    // Update Redis cache with latest order book
    await updateOrderBookCache(selectedSymbol);
    
    console.log(`[${new Date().toISOString()}] Generated ${numOrders} orders for ${selectedSymbol} at $${basePrice}`);
...
// Function to update order book cache in Redis
async function updateOrderBookCache(symbol) {
  try {
    // Get top 50 buy orders (bids) - highest prices first
    const buyOrders = await OrderBook.find({ 
      symbol, 
      type: 'buy', 
      status: 'open',
      $or: [
    ...
    // Get top 50 sell orders (asks) - lowest prices first
    const sellOrders = await OrderBook.find({ 
      symbol, 
      type: 'sell', 
      status: 'open',
      $or: [
    ...
    await redis.setex(
      ...
      5, 
      JSON.stringify({
        bids: formattedBuyOrders,
        asks: formattedSellOrders,
        timestamp: new Date().toISOString()
      })
    );
    
  } catch (error) {
    console.error('Error updating order book cache:', error);
  }
}
...
async function cleanupExpiredOrders() {
  try {
    const result = await OrderBook.updateMany(
      { 
...
// Start order generation service
function startOrderGenerationService() {
  console.log('Starting order generation service...');
  
  // Generate orders at random intervals (1-30 seconds)
  const scheduleNextGeneration = () => {
    const delay = Math.floor(Math.random() * 29000) + 1000; // 1-30 seconds
    setTimeout(async () => {
  ...
  // Clean up expired orders every 10 seconds
  setInterval(cleanupExpiredOrders, 10000);
...
// =============================================
// GET /api/market/orderbook?symbol=BTCUSD
app.get('/api/market/orderbook', async (req, res) => {
  try {
    ...
    try {
      const cachedData = await redis.get(`orderbook:${normalizedSymbol}`);
      if (cachedData) {
    ...
    const formattedBids = buyOrders.map(order => [
      order.price,
      order.amount,
    ...
    const formattedAsks = sellOrders.map(order => [
      order.price,
      order.amount,
      order.price * order.amount,
      order._id
...
// =============================================
// ENDPOINT: GET RECENT TRADES
// =============================================
// GET /api/market/trades?symbol=BTCUSD&limit=20
app.get('/api/market/trades', async (req, res) => {
  try {
    ...
    try {
      const cachedTrades = await redis.get(`trades:${normalizedSymbol}`);
      if (cachedTrades) {
        const parsed = JSON.parse(cachedTrades);
        return res.status(200).json({
          status: 'success',
          data: parsed.slice(0, limitNum),
          source: 'cache'
        });
      }
    } catch (redisError) {
    ...
    // Get from database
    const trades = await RecentTrade.find({ symbol: normalizedSymbol })
      .sort({ createdAt: -1 })
      .limit(limitNum)
      .lean();
...
app.post('/api/trade/order',
  ...
  [
    ...
    body('price').optional().isNumeric().withMessage('Price must be a number').custom(value => value > 0)
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      ...
      // Get current exchange rate
      const baseSymbol = normalizedSymbol.replace('USD', '').toLowerCase();
      let exchangeRate;
      
      try {
        exchangeRate = await fetchExchangeRate(baseSymbol);
      } catch (rateError) {
        // Try Redis cache
        const cachedRate = await redis.get(`exchange_rate:${baseSymbol}`);
        if (cachedRate) {
          exchangeRate = parseFloat(cachedRate);
        } else {
          return res.status(503).json({
            status: 'fail',
            message: 'Unable to get current exchange rate. Please try again.'
          });
        }
      }
      ...
      const roundedAssetAmount = Math.round(assetAmount * 1000000) / 1000000; // Round to 6 decimals
      ...
      const transaction = new Transaction({
        ...
        amount: amount,
        asset: baseSymbol,
        assetAmount: roundedAssetAmount,
        currency: 'USD',
        status: 'completed',
        method: 'internal',
      ...
      await recentTrade.save();

      // Invalidate caches
      await redis.del(`orderbook:${normalizedSymbol}`);
      await redis.del(`trades:${normalizedSymbol}`);
...
app.get('/api/trade/orders', protect, async (req, res) => {
  try {
    ...
    const orders = await OrderBook.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit))
      .lean();
    
    const total = await OrderBook.countDocuments(query);
...
// =============================================
// ENDPOINT: GET USER TRADE HISTORY
// =============================================
// GET /api/trade/history
app.get('/api/trade/history', protect, async (req, res) => {
  try {
...
app.delete('/api/trade/order/:orderId', protect, async (req, res) => {
  try {
    ...
    // If it was a buy order and partially filled, refund remaining amount
    if (order.type === 'buy' && order.filledAmount < order.amount) {
      const remainingAmount = order.total - (order.price * order.filledAmount);
...
// =============================================
// GET /api/market/rate?symbol=btc
app.get('/api/market/rate', async (req, res) => {
  try {
    ...
    // Try Redis first
    const cachedRate = await redis.get(`exchange_rate:${normalizedSymbol}`);
    if (cachedRate) {
      return res.status(200).json({
        status: 'success',
        data: {
    ...
    const rate = await fetchExchangeRate(normalizedSymbol);
    
    // Cache in Redis
    await redis.setex(`exchange_rate:${normalizedSymbol}`, 60, rate.toString());
...
// =============================================
// START THE ORDER GENERATION SERVICE
// =============================================
// Call this after database connection is established
setTimeout(() => {
  startOrderGenerationService();
}, 5000);
