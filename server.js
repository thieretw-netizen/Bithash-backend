...
// =============================================
// COMPLETE SNIPPET B - TRADING ENDPOINTS & WEBSOCKETS
// =============================================
// This file contains ALL missing endpoints and WebSocket connections
// for the BitHash trading platform.
// 
// NOTE: axios and uuid are already required in Snippet A
...
const REDIS_PRICE_KEY = 'current_prices';
const REDIS_ORDERBOOK_KEY = 'orderbook:';
const REDIS_CANDLES_KEY = 'candles:';

const TIMEFRAMES = ['1s', '15m', '1h', '4h', '1d', '1w'];
const QUOTE_ASSETS = ['USDT', 'USDC', 'USDQ', 'USDR', 'EURC', 'USD', 'BNB', 'BTC'];
const BASE_ASSETS = ['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'ADA', 'DOGE', 'AVAX', 'DOT', 'LINK', 'MATIC', 'SHIB', 'TRX', 'UNI', 'ATOM', 'XLM', 'FIL', 'VET', 'ALGO', 'MANA', 'SAND', 'AXS', 'AAVE', 'EOS', 'MKR', 'DASH', 'XTZ', 'FTM', 'NEAR', 'GRT', 'HBAR', 'QNT', 'THETA', 'ICP', 'FLOW', 'BCH', 'WBTC', 'LTC', 'XMR', 'ETC', 'ZEC', 'NEO', 'IOTA'];
...
const startPriceAggregator = async () => {
    ...
    setInterval(checkBinanceHealth, 5000);
};
...
const connectToBinanceWebSocket = () => {
    ...
    binanceWs.on('message', async (data) => {
        try {
            ...
            await processBinanceMessage(parsed);
        } catch (err) {
    ...
    binanceWs.on('close', () => {
        ...
        setTimeout(connectToBinanceWebSocket, delay);
    });
...
const checkBinanceHealth = () => {
    if (!isBinanceConnected) {
        console.warn('⚠️ Binance WebSocket is disconnected');
    }
};

const processBinanceMessage = async (message) => {
    if (!message) return;
    
    if (message.e === '24hrTicker') {
        await processTickerUpdate(message);
    } else if (message.e === 'depthUpdate') {
        await processDepthUpdate(message);
    } else if (message.e === 'trade') {
        await processTradeUpdate(message);
    } else if (message.e === 'kline') {
        await processKlineUpdate(message);
    }
};

const processTickerUpdate = async (data) => {
    const symbol = data.s;
    ...
    await redis.hset(REDIS_PRICE_KEY, symbol, JSON.stringify(tickerData));
    ...
    await updateMarketPairInDB(symbol, tickerData);
};

const updateMarketPairInDB = async (symbol, tickerData) => {
    try {
        const baseAsset = symbol.replace(/USDT|USDC|USDQ|USDR|EURC|USD|BNB|BTC$/, '');
        ...
        console.error('Error updating market pair:', err);
    }
};

const processDepthUpdate = async (data) => {
    const symbol = data.s;
    ...
    await redis.set(`${REDIS_ORDERBOOK_KEY}${symbol}`, JSON.stringify(orderbookData), 'EX', 5);
...
const processTradeUpdate = async (data) => {
    const trade = {
        symbol: data.s,
...
const processKlineUpdate = async (data) => {
    const kline = data.k;
    const symbol = data.s;
    ...
    const redisKey = `${REDIS_CANDLES_KEY}${symbol}:${interval}`;
    const score = kline.t;
    
    await redis.zadd(redisKey, score, JSON.stringify(candle));
    ...
    await redis.publish('market:candle', JSON.stringify({
        type: 'candles',
        symbol,
        interval,
        candle
...
const setupMarketWebSocket = (server) => {
    ...
    const subscribeClientToRedis = async (client, pair, channels) => {
        const subscriber = redis.duplicate();
        client.subscriber = subscriber;
        client.subscribedChannels = channels;
        client.currentPair = pair;
        
        for (const channel of channels) {
            await subscriber.subscribe(`market:${channel}`);
        }
        
        subscriber.on('message', (channel, message) => {
            if (client.readyState === WebSocket.OPEN) {
                const data = JSON.parse(message);
                if (!pair || data.symbol === pair || data.symbol === pair.toUpperCase()) {
                    client.send(message);
                }
            }
        });
    ...
    marketWss.on('connection', async (ws, req) => {
        ...
        ws.on('message', async (message) => {
            try {
                ...
                if (data.type === 'subscribe') {
                    ...
                    await subscribeClientToRedis(ws, data.pair, data.channels);
    ...
    heartbeatInterval = setInterval(() => {
        marketClients.forEach(client => {
            if (!client.isAlive) {
                if (client.subscriber) client.subscriber.quit();
...
const setupTickerWebSocket = (server) => {
    ...
    const startTickerBroadcast = async () => {
        const subscriber = redis.duplicate();
        await subscriber.subscribe('market:ticker');
        
        subscriber.on('message', (channel, message) => {
            const data = JSON.parse(message);
            broadcastTickerUpdate(data.symbol, data.price, data.priceChangePercent);
        });
    };
...
// GET /api/market/candles?symbol=<pair>&interval=<timeframe>&limit=200
app.get('/api/market/candles', async (req, res) => {
    try {
        ...
        const redisKey = `${REDIS_CANDLES_KEY}${symbol}:${interval}`;
        const candles = await redis.zrevrangebyscore(redisKey, '+inf', '-inf', 'LIMIT', 0, parseInt(limit));
        
        const formattedCandles = candles.map(candle => {
            const c = JSON.parse(candle);
            return {
...
// GET /api/market/orderbook?symbol=<pair>&limit=100
app.get('/api/market/orderbook', async (req, res) => {
    try {
        ...
        const orderbookData = await redis.get(`${REDIS_ORDERBOOK_KEY}${symbol}`);
...
// GET /api/market/ticker/24hr?symbol=<pair>
app.get('/api/market/ticker/24hr', async (req, res) => {
    try {
        const { symbol } = req.query;
        
        if (symbol) {
            const tickerData = await redis.hget(REDIS_PRICE_KEY, symbol);
            
            if (tickerData) {
                const data = JSON.parse(tickerData);
                res.json({
                    status: 'success',
                    symbol: data.symbol,
                    priceChange: data.price - (data.price / (1 + data.priceChangePercent / 100)),
                    priceChangePercent: data.priceChangePercent,
                    lastPrice: data.price,
                    highPrice: data.high24h,
                    lowPrice: data.low24h,
                    volume: data.volume24h,
                    quoteVolume: data.quoteVolume24h,
                    openPrice: data.price / (1 + data.priceChangePercent / 100)
                });
            } else {
                const dbTicker = await Ticker24hr.findOne({ symbol });
                if (dbTicker) {
                    res.json({ status: 'success', ...dbTicker.toObject() });
                } else {
                    res.json({ status: 'success', symbol, lastPrice: 0 });
                }
            }
        } else {
            const allTickers = await redis.hgetall(REDIS_PRICE_KEY);
            const tickers = Object.values(allTickers).map(t => JSON.parse(t));
            res.json({
                status: 'success',
                data: tickers
            });
...
// GET /api/market/trades?symbol=<pair>&limit=50
...
app.get('/api/market/pairs', async (req, res) => {
    try {
        ...
        const pairsWithLogos = pairs.map(pair => ({
            symbol: pair.symbol,
            base: pair.baseAsset,
...
// GET /api/trading/pairlimits?symbol=<pair>
app.get('/api/trading/pairlimits', async (req, res) => {
    try {
        ...
        const baseAsset = symbol.replace(/USDT|USDC|USDQ|USDR|EURC|USD|BNB|BTC$/, '');
        const logoUrl = await getAssetLogo(baseAsset);
        
        res.json({
            status: 'success',
            maxBuyAmount: pair?.maxQty || 1000,
            maxSellAmount: pair?.maxQty || 1000,
            minAmount: pair?.minQty || 0.0001,
            logoUrl
        });
    } catch (err) {
        ...
        res.status(500).json({
            status: 'error',
            message: 'Failed to fetch pair limits'
        });
    }
});

// GET /api/asset/info?symbol=<base_asset>
    ...
    try {
        ...
        if (!assetInfo) {
            try {
                ...
                if (response.data) {
                    assetInfo = await AssetInfo.findOneAndUpdate(
                        { symbol },
                        {
                            ...
                            fullyDilutedMarketCap: response.data.market_data?.fully_diluted_valuation?.usd,
                ...
                console.log('CoinGecko API failed, returning default values');
            }
        }
...
const getAssetLogo = async (symbol) => {
    const cached = await redis.hget('asset_logos', symbol);
    if (cached) return cached;
    
    const assetInfo = await AssetInfo.findOne({ symbol });
    if (assetInfo?.logo) return assetInfo.logo;
    
    const coingeckoId = getCoingeckoId(symbol);
    const logoUrl = `https://coin-images.coingecko.com/coins/images/1/${coingeckoId}.png`;
    
    await redis.hset('asset_logos', symbol, logoUrl);
    return logoUrl;
};

// GET /api/asset/logo?symbol=<base_asset>
app.get('/api/asset/logo', async (req, res) => {
    try {
        const { symbol } = req.query;
        const logoUrl = await getAssetLogo(symbol);
        ...
        res.status(500).json({
            status: 'error',
            message: 'Failed to fetch asset logo'
        });
    }
});

// GET /api/asset/extra?symbol=<base_asset>
app.get('/api/asset/extra', async (req, res) => {
    try {
        ...
        const tags = assetInfo?.tags || getDefaultTags(symbol);
        const networks = assetInfo?.networks || [symbol, 'ERC-20', 'BEP-20'];
        ...
        res.status(500).json({
            status: 'error',
            message: 'Failed to fetch asset extra data'
        });
    }
});
...
const getDefaultTags = (symbol) => {
    const tagsMap = {
        ...
        'ADA': ['Smart Contracts', 'Research', 'Proof of Stake'],
        'DOGE': ['Meme', 'Payments', 'Community'],
        'AVAX': ['Subnets', 'DeFi', 'High Performance'],
        'DOT': ['Parachains', 'Interoperability', 'Governance'],
        'LINK': ['Oracle', 'DeFi', 'Data Feeds']
    };
    return tagsMap[symbol] || ['Cryptocurrency', 'Digital Asset'];
};

// GET /api/trading/data?pair=<pair>
    ...
    try {
        ...
        if (!tradingData) {
            tradingData = await TradingData.create({
                ...
                netFlow: Array(7).fill(0).map(() => (Math.random() - 0.5) * 100),
                inflow24h: Math.random() * 1000000,
                outflow24h: Math.random() * 1000000,
                netFlow24h: (Math.random() - 0.5) * 500000
            });
        }
...
app.get('/api/analysis', async (req, res) => {
    try {
        ...
        if (!analysis) {
            analysis = await AnalysisData.create({
                symbol: pair,
                longShortRatio: 0.5 + Math.random(),
                marginData: Math.random() * 1000000,
...
app.get('/api/users/assets', protect, async (req, res) => {
    try {
        ...
        if (!userAssets) {
            userAssets = await UserAssetBalance.create({
                user: req.user._id,
                balances: {},
                history: []
            });
        ...
        const assets = Object.entries(userAssets.balances)
            .filter(([_, balance]) => balance > 0)
            .map(([symbol, balance]) => ({
                symbol: symbol.toUpperCase(),
                balance,
                value: balance * (currentPrices[symbol] || 0)
            }));
...
// GET /api/trading/orders?symbol=<pair>
...
// GET /api/trading/trades?symbol=<pair>
...
// GET /api/trading/positions?symbol=<pair>
app.get('/api/trading/positions', protect, async (req, res) => {
    try {
        const { symbol } = req.query;
        
        let query = { user: req.user._id, status: 'open' };
        if (symbol) query.symbol = symbol;
...
app.post('/api/trading/orders/buy', protect, async (req, res) => {
    try {
        ...
        const currentPriceData = await redis.hget(REDIS_PRICE_KEY, symbol);
        const currentPrice = currentPriceData ? JSON.parse(currentPriceData).price : pair.price;
        ...
        const userUsdtBalance = await getUserAssetBalance(req.user._id, 'usdt');
        
        if (totalWithFee > userUsdtBalance) {
            return res.status(400).json({
                status: 'fail',
                message: `Insufficient balance. Need ${totalWithFee.toFixed(2)} USDT, have ${userUsdtBalance.toFixed(2)} USDT`
            });
        }
        ...
        if (type === 'market') {
            await executeMarketBuy(req.user._id, symbol, amount, orderPrice, fee, orderId);
            ...
            await createTrade(req.user._id, orderId, symbol, 'buy', orderPrice, amount, fee);
        }
        ...
        res.json({
            status: 'success',
            data: { orderId, status: order.status, message: 'Order placed successfully' }
        });
        
        io.to(`user_${req.user._id}`).emit('balance_update', await getUserBalances(req.user._id));
        io.to(`user_${req.user._id}`).emit('order_update', { orderId, status: order.status });
...
app.post('/api/trading/orders/sell', protect, async (req, res) => {
    try {
        ...
        const userBaseBalance = await getUserAssetBalance(req.user._id, pair.baseAsset.toLowerCase());
        
        if (amountAfterFee > userBaseBalance) {
            return res.status(400).json({
                status: 'fail',
                message: `Insufficient balance. Need ${amountAfterFee.toFixed(8)} ${pair.baseAsset}, have ${userBaseBalance.toFixed(8)} ${pair.baseAsset}`
            });
        }
        ...
        if (type === 'market') {
            await executeMarketSell(req.user._id, symbol, amount, orderPrice, fee, orderId);
            order.executedQty = amount;
            order.remainingQty = 0;
            order.status = 'filled';
            order.avgPrice = orderPrice;
            ...
            await createTrade(req.user._id, orderId, symbol, 'sell', orderPrice, amount, fee);
        }
...
app.post('/api/trading/orders/cancel', protect, async (req, res) => {
    try {
        ...
        io.to(`user_${req.user._id}`).emit('order_update', { orderId, status: 'cancelled' });
...
app.post('/api/trading/orders/cancel-all', protect, async (req, res) => {
    try {
        ...
        let query = { user: req.user._id, status: { $in: ['new', 'partial', 'pending'] } };
        if (symbol) query.symbol = symbol;
        ...
        io.to(`user_${req.user._id}`).emit('orders_cancelled', { count: result.modifiedCount });
...
app.post('/api/trading/positions/close', protect, async (req, res) => {
    try {
        ...
        const currentPriceData = await redis.hget(REDIS_PRICE_KEY, position.symbol);
        const currentPrice = currentPriceData ? JSON.parse(currentPriceData).price : position.entryPrice;
        ...
        await updateUserBalanceAfterClose(req.user._id, position, pnl);
...
const getUserAssetBalance = async (userId, asset) => {
    const userAssets = await UserAssetBalance.findOne({ user: userId });
    if (!userAssets) return 0;
    return userAssets.balances[asset] || 0;
};

const updateUserAssetBalance = async (userId, asset, amount, type, price, transactionId) => {
    let userAssets = await UserAssetBalance.findOne({ user: userId });
    ...
    const currentBalance = userAssets.balances[asset] || 0;
    const newBalance = type === 'add' ? currentBalance + amount : currentBalance - amount;
    
    await UserAssetBalance.updateOne(
        { user: userId },
        {
            $set: { [`balances.${asset}`]: newBalance, lastUpdated: new Date() },
            $push: {
                history: {
                    asset,
...
const executeMarketBuy = async (userId, symbol, amount, price, fee, orderId) => {
    const pair = await MarketPair.findOne({ symbol });
    const quoteAsset = pair?.quoteAsset?.toLowerCase() || 'usdt';
    const baseAsset = pair?.baseAsset?.toLowerCase() || 'btc';
    
    await updateUserAssetBalance(userId, quoteAsset, amount * price + fee, 'subtract', price, orderId);
    await updateUserAssetBalance(userId, baseAsset, amount, 'add', price, orderId);
};

const executeMarketSell = async (userId, symbol, amount, price, fee, orderId) => {
    const pair = await MarketPair.findOne({ symbol });
    const quoteAsset = pair?.quoteAsset?.toLowerCase() || 'usdt';
    const baseAsset = pair?.baseAsset?.toLowerCase() || 'btc';
    
    await updateUserAssetBalance(userId, baseAsset, amount, 'subtract', price, orderId);
    await updateUserAssetBalance(userId, quoteAsset, amount * price - fee, 'add', price, orderId);
};

const createTrade = async (userId, orderId, symbol, side, price, qty, commission) => {
...
const updateUserBalanceAfterClose = async (userId, position, pnl) => {
    const user = await User.findById(userId);
    if (user) {
...
const getUserBalances = async (userId) => {
    const user = await User.findById(userId).select('balances');
    return user?.balances || { main: 0, active: 0, matured: 0, savings: 0, loan: 0 };
};
