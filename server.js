...
const detectAndSetUserPreferencesFromIP = async (userId, req) => {
  try {
    ...
    if (existingPrefs && existingPrefs.language && existingPrefs.currency) {
      console.log(`User ${userId} already has preferences saved, skipping IP detection`);
      return;
    }
    ...
    try {
      // First try ipapi.co (free, no token required, fast)
      ...
      if (response.data && !response.data.error) {
        ...
        if (countryCode && langMap[countryCode]) {
          detectedLanguage = langMap[countryCode];
        }
        
        console.log(`IP detection (ipapi.co) for ${clientIp}: currency=${detectedCurrency}, language=${detectedLanguage}`);
      } else {
      ...
      try {
        const ipinfoToken = process.env.IPINFO_TOKEN || 'b56ce6e91d732d';
        const response = await axios.get(`https://ipinfo.io/${clientIp}?token=${ipinfoToken}`, { timeout: 3000 });
        if (response.data) {
          const country = response.data.country;
          // Map country to currency (simplified mapping)
          const countryCurrencyMap = {
            'US': 'USD', 'GB': 'GBP', 'EU': 'EUR', 'JP': 'JPY', 'CA': 'CAD',
            'AU': 'AUD', 'CH': 'CHF', 'CN': 'CNY', 'IN': 'INR', 'BR': 'BRL',
            'MX': 'MXN', 'KR': 'KRW', 'RU': 'RUB', 'ZA': 'ZAR', 'TR': 'TRY',
            'SG': 'SGD', 'NZ': 'NZD', 'SE': 'SEK', 'NO': 'NOK', 'DK': 'DKK'
          };
          if (country && countryCurrencyMap[country]) {
            detectedCurrency = countryCurrencyMap[country];
          }
          
          // Map country to language
          const langMap = {
            'US': 'en', 'GB': 'en', 'AU': 'en', 'CA': 'en', 'ES': 'es', 'MX': 'es',
            'FR': 'fr', 'DE': 'de', 'IT': 'it', 'PT': 'pt', 'BR': 'pt', 'JP': 'ja',
            'CN': 'zh', 'RU': 'ru', 'KR': 'ko', 'NL': 'nl', 'PL': 'pl', 'TR': 'tr'
          };
          if (country && langMap[country]) {
            detectedLanguage = langMap[country];
          }
          
          console.log(`IP detection (ipinfo.io) for ${clientIp}: currency=${detectedCurrency}, language=${detectedLanguage}`);
        }
      } catch (secondError) {
        console.log('Both IP detection services failed, using defaults');
      }
    }

    // Ensure currency is one of the supported ones
    const supportedCurrencies = ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'CNY', 'INR', 'BRL', 'MXN', 'KRW', 'RUB', 'ZAR', 'TRY', 'SGD', 'NZD', 'SEK', 'NOK', 'DKK'];
    if (!supportedCurrencies.includes(detectedCurrency)) {
      detectedCurrency = 'USD';
    }

    // Ensure language is one of the supported ones
    const supportedLanguages = ['en', 'es', 'fr', 'de', 'it', 'pt', 'ja', 'zh', 'ru', 'ko', 'nl', 'pl', 'tr', 'sv', 'no', 'da', 'fi', 'el', 'cs', 'hu', 'ro'];
    ...
    console.log(`✅ IP-based preferences saved for user ${userId}: currency=${detectedCurrency}, language=${detectedLanguage}`);
    
    return { currency: detectedCurrency, language: detectedLanguage };
  } catch (err) {
    console.error('Error detecting and setting IP-based preferences:', err);
    return null;
  }
};

// =============================================
// REAL-TIME BALANCE FLUCTUATION WITH CRYPTO API
// =============================================
...
const fetchAllCryptoPrices = async () => {
  const now = Date.now();
  if (now - lastPriceFetchTime < PRICE_CACHE_TTL && Object.keys(cryptoPriceCache).length > 0) {
    return cryptoPriceCache;
  }
  
  try {
    // Fetch from CoinGecko for 30+ cryptocurrencies
    const assetIds = [
      'bitcoin', 'ethereum', 'tether', 'binancecoin', 'solana', 'usd-coin',
      'ripple', 'dogecoin', 'cardano', 'shiba-inu', 'avalanche-2', 'polkadot',
      'tron', 'chainlink', 'matic-network', 'wrapped-bitcoin', 'litecoin',
      'near', 'uniswap', 'bitcoin-cash', 'stellar', 'cosmos', 'monero',
      'flow', 'vechain', 'filecoin', 'theta-network', 'hedera-hashgraph',
      'fantom', 'tezos'
    ];
    ...
    if (response.data) {
      cryptoPriceCache = response.data;
      lastPriceFetchTime = now;
      console.log('Fetched latest crypto prices for balance fluctuations');
    }
  } catch (err) {
    console.error('Failed to fetch crypto prices for balance fluctuations:', err.message);
  }
  
  return cryptoPriceCache;
};

// Function to calculate real-time USD value of user's asset balances
const calculateRealTimeAssetValue = async (userAssetBalances, cryptoPrices) => {
  let totalValue = 0;
  const assetToIdMap = {
  ...
  for (const [asset, balance] of Object.entries(userAssetBalances)) {
    if (balance > 0 && assetToIdMap[asset.toLowerCase()]) {
      const price = cryptoPrices[assetToIdMap[asset.toLowerCase()]]?.usd || 0;
    ...
    if (userAssetBalDoc) {
      ...
      for (const [asset, balance] of Object.entries(userAssetBalDoc.balances)) {
        if (balance > 0) {
          ...
          const avgPrice = 0; // Would need to calculate from buy history
          ...
          assetsWithValues.push({
            ...
            unrealizedPnl: (currentPrice - avgPrice) * balance,
    ...
    const priceInterval = setInterval(async () => {
      try {
        ...
        if (userAssetBalDoc) {
          ...
          // Send PnL updates (simulated based on price changes)
          // In production, track historical prices per user
...
// Admin Approve Deposit Endpoint - FIXED VERSION with asset balance handling
app.post('/api/admin/deposits/:id/approve', adminProtect, [
  ...
  try {
    ...
    // Update user balance - add to main balance
    user.balances.main += deposit.amount;
    await user.save();
    
    // If payment method was crypto (not BANK or CARD), update asset balances
    const isCryptoPayment = deposit.method !== 'BANK' && deposit.method !== 'CARD' && deposit.method !== 'INTERNAL';
    let assetSymbol = null;
    let cryptoAmount = null;
    
    if (isCryptoPayment && deposit.asset) {
      assetSymbol = deposit.asset.toLowerCase();
      cryptoAmount = deposit.assetAmount || (deposit.amount / (deposit.exchangeRateAtTime || await getCryptoPrice(deposit.asset)));
      ...
      // Add to the specific crypto balance
      if (userAssetBalance.balances[assetSymbol] !== undefined) {
        userAssetBalance.balances[assetSymbol] = (userAssetBalance.balances[assetSymbol] || 0) + cryptoAmount;
      } else {
        userAssetBalance.balances[assetSymbol] = cryptoAmount;
      }
      ...
      userAssetBalance.history.push({
        ...
        price: deposit.exchangeRateAtTime || await getCryptoPrice(deposit.asset),
      ...
      console.log(`✅ Added ${cryptoAmount} ${assetSymbol} to user ${user.email}'s asset balance`);
    }
...
app.post('/api/admin/withdrawals/:id/approve', adminProtect, [
  ...
  try {
    ...
    // If withdrawal is for a crypto asset, remove from user's asset balance
    const isCryptoWithdrawal = withdrawal.method !== 'BANK' && withdrawal.method !== 'CARD' && withdrawal.method !== 'INTERNAL';
    let assetSymbol = null;
    let cryptoAmount = null;
    
    if (isCryptoWithdrawal && withdrawal.asset) {
      assetSymbol = withdrawal.asset.toLowerCase();
      cryptoAmount = withdrawal.assetAmount || (withdrawal.amount / (cryptoPrice || withdrawal.exchangeRateAtTime || 1));
      
      // Update UserAssetBalance - remove the withdrawn amount
      let userAssetBalance = await UserAssetBalance.findOne({ user: user._id });
      if (userAssetBalance && userAssetBalance.balances[assetSymbol] !== undefined) {
        const currentBalance = userAssetBalance.balances[assetSymbol] || 0;
        if (currentBalance >= cryptoAmount) {
          userAssetBalance.balances[assetSymbol] = currentBalance - cryptoAmount;
          ...
          userAssetBalance.history.push({
            ...
            balance: userAssetBalance.balances[assetSymbol],
            usdValue: withdrawal.amount,
            price: cryptoPrice || withdrawal.exchangeRateAtTime,
          ...
          console.log(`✅ Removed ${cryptoAmount} ${assetSymbol} from user ${user.email}'s asset balance`);
        } else {
          console.warn(`⚠️ Insufficient ${assetSymbol} balance for withdrawal: ${currentBalance} < ${cryptoAmount}`);
        }
      }
    }
    ...
    if (io) {
      ...
      if (assetSymbol) {
        io.to(`user_${withdrawal.user._id}`).emit('asset_balance_removed', {
          symbol: assetSymbol,
          amount: cryptoAmount
        });
      }
...
// POST /api/users/preferences/save - Save user preferences (language and fiat currency)
app.post('/api/users/preferences/save', protect, [
  ...
  try {
    ...
    // Update fiat currency if provided
    if (fiatCurrency) {
      const supportedCurrencies = ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'CNY', 'INR', 'BRL', 'MXN', 'KRW', 'RUB', 'ZAR', 'TRY', 'SGD', 'NZD', 'SEK', 'NOK', 'DKK'];
      if (supportedCurrencies.includes(fiatCurrency.toUpperCase())) {
        preferences.currency = fiatCurrency.toUpperCase();
      } else {
    ...
    if (io) {
      io.to(`user_${userId}`).emit('preferences_update', {
        ...
        fiatCurrency: preferences.currency,
...
app.get('/api/convert/assets', protect, async (req, res) => {
  try {
    ...
    // Get all supported assets with their current prices
    const assetList = [
    ...
    const assetsWithPrices = assetList.map(asset => {
      const coinId = assetToIdMap[asset.symbol];
      const currentPrice = prices[coinId]?.usd || 0;
      const userBalance = userAssetBalance?.balances[asset.symbol] || 0;
      
      return {
        ...asset,
        currentPrice: currentPrice,
        userBalance: userBalance,
        userBalanceUsd: userBalance * currentPrice
      };
    });
    ...
    res.status(500).json({
      status: 'error',
      message: 'Failed to fetch available assets for conversion'
    });
  }
});

// POST /api/convert - Convert one crypto to another
app.post('/api/convert', protect, [
  ...
  try {
    ...
    if (!userAssetBalance) {
      return res.status(400).json({
        status: 'fail',
        message: `You don't have any ${fromAssetLower.toUpperCase()} balance`
      });
    }
    
    // Check if user has enough balance
    const currentBalance = userAssetBalance.balances[fromAssetLower] || 0;
    if (currentBalance < amount) {
      return res.status(400).json({
        status: 'fail',
        message: `Insufficient ${fromAssetLower.toUpperCase()} balance. You have ${currentBalance} ${fromAssetLower.toUpperCase()}`
      });
    }
    ...
    const fromCoinId = assetToIdMap[fromAssetLower];
    const toCoinId = assetToIdMap[toAssetLower];
    
    if (!fromCoinId || !toCoinId) {
      return res.status(400).json({
        status: 'fail',
        message: 'Invalid asset for conversion'
      });
    }
    ...
    if (fromPrice === 0 || toPrice === 0) {
      return res.status(503).json({
        status: 'fail',
        message: 'Unable to fetch current prices for conversion. Please try again.'
      });
    }
    ...
    // Apply 0.1% conversion fee
    const conversionFee = usdValue * 0.001;
    const netUsdValue = usdValue - conversionFee;
    const netToAmount = netUsdValue / toPrice;
    
    // Update balances: remove from source asset
    userAssetBalance.balances[fromAssetLower] = currentBalance - amount;
    
    // Add to target asset
    const currentToBalance = userAssetBalance.balances[toAssetLower] || 0;
    userAssetBalance.balances[toAssetLower] = currentToBalance + netToAmount;
    
    // Add to history for both assets
    userAssetBalance.history.push({
    ...
    await Transaction.create({
      ...
      assetAmount: netToAmount,
      currency: 'USD',
      status: 'completed',
      method: 'INTERNAL',
      reference: `CONV-${Date.now()}-${Math.floor(Math.random() * 10000)}-buy`,
      details: {
        conversionType: 'crypto_to_crypto',
        fromAsset: fromAsset,
        toAsset: toAsset,
    ...
    if (io) {
      io.to(`user_${userId}`).emit('asset_balances_update', [
        { symbol: fromAssetLower, balance: userAssetBalance.balances[fromAssetLower] },
        { symbol: toAssetLower, balance: userAssetBalance.balances[toAssetLower] }
      ]);
    }
    
    res.status(200).json({
      status: 'success',
      message: `Successfully converted ${amount} ${fromAsset.toUpperCase()} to ${netToAmount.toFixed(8)} ${toAsset.toUpperCase()}`,
      data: {
        fromAsset: fromAsset,
        toAsset: toAsset,
        fromAmount: amount,
        ...
        toPrice: toPrice,
        fee: conversionFee,
        newFromBalance: userAssetBalance.balances[fromAssetLower],
        newToBalance: userAssetBalance.balances[toAssetLower]
      }
    });
    
  } catch (err) {
    console.error('Error during crypto conversion:', err);
    res.status(500).json({
      status: 'error',
...
// GET /api/fiat-currencies - Get all supported fiat currencies with real-time exchange rates
app.get('/api/fiat-currencies', async (req, res) => {
  try {
    // Base currency is USD
    const baseCurrency = 'USD';
    
    // List of all supported currencies (all currencies in the world)
    const allCurrencies = [
      ...
      { code: 'PLN', name: 'Polish Złoty', symbol: 'zł', flag: 'https://flagcdn.com/w40/pl.png' },
    ...
    try {
      // Try exchangerate-api.com (free, no API key required for limited requests)
      ...
      console.log('Exchangerate-api.com failed, trying fallback...');
      ...
      try {
        ...
        if (response.data && response.data.rates) {
          exchangeRates = response.data.rates;
          ratesFetched = true;
          console.log('Fetched real-time exchange rates from frankfurter.app');
        }
      } catch (secondError) {
        console.log('Frankfurter.app failed, using cached rates');
      }
    }
    
    // If rates couldn't be fetched, use approximate fallback rates (not mock, but based on real market)
    if (!ratesFetched) {
      // These are approximate real-world rates as of current market
      exchangeRates = {
        'USD': 1, 'EUR': 0.92, 'GBP': 0.79, 'JPY': 150.5, 'CAD': 1.36,
        'AUD': 1.52, 'CHF': 0.91, 'CNY': 7.24, 'INR': 83.2, 'BRL': 5.05,
        'MXN': 17.1, 'KRW': 1330, 'RUB': 92.5, 'ZAR': 18.9, 'TRY': 32.4,
        'SGD': 1.35, 'NZD': 1.66, 'SEK': 10.6, 'NOK': 10.8, 'DKK': 6.89,
        'PLN': 4.02, 'THB': 36.2, 'IDR': 15700, 'HKD': 7.82, 'MYR': 4.73,
        'PHP': 56.3, 'VND': 24900, 'AED': 3.67, 'SAR': 3.75, 'ILS': 3.70
      };
      console.log('Using approximate exchange rates (API unavailable)');
    }
    
    // Build the currencies list with exchange rates
    const currenciesWithRates = allCurrencies.map(currency => ({
      ...currency,
      exchangeRate: exchangeRates[currency.code] || 1
    }));
    ...
    // Return a basic list as fallback (still with real exchange rates from cache if available)
    res.status(500).json({
      status: 'error',
  ...
  try {
    ...
    for (const investment of maturedInvestments) {
      try {
        ...
        // Transfer balances - from active to matured
        user.balances.active -= investment.amount;
        user.balances.matured += totalReturn;
...
app.use(async (req, res, next) => {
  // Only apply to authenticated users on first visit
  if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
    try {
      const token = req.headers.authorization.split(' ')[1];
      const decoded = verifyJWT(token);
      if (decoded && decoded.id) {
        ...
        const existingPrefs = await UserPreference.findOne({ user: decoded.id });
        if (!existingPrefs) {
          // This is a first-time visitor, detect and set preferences from IP
          await detectAndSetUserPreferencesFromIP(decoded.id, req);
        }
      }
    } catch (err) {
      // Token might be invalid, just continue
    }
  }
  next();
});
...
httpServer.listen(PORT, () => {
  ...
  console.log(`🔌 WebSocket server initialized for real-time balance updates`);
  console.log(`🌍 IP-based preference detection enabled for first-time visitors`);
});
