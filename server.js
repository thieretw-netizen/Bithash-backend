  ...
  try {
    ...
    const brandHeader = `
      <div style="text-align: center; padding: 30px 20px 20px 20px; background: linear-gradient(135deg, #0B0E11 0%, #11151C 100%);">
        ...
        <p style="color: #6C7480; font-size: 12px; margin: 5px 0;">${timestamp}</p>
      </div>
    `;
...
// Enhanced Login Endpoint with OTP - FIXED email handling with failed attempt tracking
app.post('/api/auth/login', [
  ...
  try {
    ...
    const deviceInfo = await getUserDeviceInfo(req);
    const accurateInfo = getAccurateDeviceInfo(req.headers['user-agent']);
    ...
    await UserLog.create({
      ...
      metadata: {
        ...
        otpSent: true,
        accurateDeviceInfo: accurateInfo
      }
    });
    ...
    try {
      await sendAutomatedEmail(user, 'login_success', {
        name: user.firstName,
        device: `${accurateInfo.device} (${accurateInfo.model}) - ${accurateInfo.os} - ${accurateInfo.browser}`,
        location: deviceInfo.location,
        ip: deviceInfo.ip,
        timestamp: new Date()
      });
...
app.post('/api/withdrawals/asset', protect, async (req, res) => {
    try {
        ...
        const assetBalanceInWallet = userAssetBalance.balances[assetLower] || 0;
        ...
        const assetAmountNeeded = amount / assetPrice;
        
        if (assetBalanceInWallet < assetAmountNeeded) {
            return res.status(400).json({
                status: 'error',
                message: `Insufficient ${asset.toUpperCase()} balance. Required: ${assetAmountNeeded.toFixed(8)} ${asset.toUpperCase()}, Available: ${assetBalanceInWallet.toFixed(8)} ${asset.toUpperCase()}`
            });
        }
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') console.log(`Gas fee calculation: BTC fee: ${btcGasFeeAmount} BTC, BTC price: $${btcPrice}, Gas fee in USD: $${gasFeeInUsd.toFixed(2)}`);
        ...
        userAssetBalance.balances[assetLower] -= assetAmountNeeded;
        ...
        return res.status(201).json({
            ...
            message: `Withdrawal request submitted successfully. Gas fee of $${gasFeeInUsd.toFixed(2)} deducted from main wallet.`
        });
