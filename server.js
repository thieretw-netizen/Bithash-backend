...
// =============================================
// ENHANCED PRICE AGGREGATOR WORKER - ENTERPRISE GRADE
// Single source of truth - ALL Binance pairs hot in Redis
// Millisecond response times for any pair request
// =============================================
...
const AGGREGATOR_CONFIG = {
    ...
    BATCH_SUBSCRIBE_SIZE: 100,
    SUBSCRIBE_DELAY_MS: 50,
    CANDLE_LIMIT_PER_SYMBOL: 500,
    MAX_ORDER_BOOK_DEPTH: 100,
    MAX_RECENT_TRADES: 200,
    PRICE_UPDATE_BROADCAST: true,
...
const REDIS_KEYS = {
    ...
    ORDERBOOK_DEPTH: (symbol, depth) => `market:orderbook:${symbol}:depth${depth}`,
    CANDLES: (symbol, interval) => `market:candles:${symbol}:${interval}`,
    TRADES: (symbol) => `market:trades:${symbol}`,
    LAST_PRICE: (symbol) => `market:price:${symbol}:last`,
    PRICE_CHANGE: (symbol) => `market:price:${symbol}:change`,
    VOLUME_24H: (symbol) => `market:volume:${symbol}:24h`,
    
    // Asset Info
    ASSET_INFO: (symbol) => `asset:info:${symbol}`,
    ASSET_EXTRA: (symbol) => `asset:extra:${symbol}`,
    ASSET_LOGO: (symbol) => `asset:logo:${symbol}`,
    
    // Market Lists
    ALL_PAIRS: 'market:all:pairs',
    ALL_PAIRS_BY_QUOTE: (quote) => `market:pairs:quote:${quote}`,
    QUOTE_ASSETS: 'market:quote:assets',
    ALL_SYMBOLS: 'market:all:symbols',
    ...
    HIGHEST_VOLUME: 'market:top:volume',
    
    // Health & Status
    AGGREGATOR_STATUS: 'aggregator:status',
    LAST_UPDATE: (symbol) => `market:update:${symbol}`,
    WEBSOCKET_STATUS: 'aggregator:websocket:status',
    
    // Indexes for fast lookups
    PRICE_INDEX: 'market:price:index',
    SYMBOL_TO_BASE: 'market:symbol:base',
    SYMBOL_TO_QUOTE: 'market:symbol:quote'
};
...
const aggregatorEvents = new EventEmitter();

// =============================================
// ENHANCED PAIR FETCHING - ALL BINANCE PAIRS
// =============================================
...
async function fetchAllBinancePairs() {
    ...
    try {
        ...
        const allPairs = [];
        const quoteAssetsMap = new Map();
        const baseAssetsSet = new Set();
        
        // ALL quote assets (not just major ones)
        const allQuoteAssets = new Set();
        ...
        for (const symbol of allSymbols) {
            if (symbol.status === 'TRADING') {
                const pair = {
                    ...
                    stepSize: parseFloat(symbol.filters.find(f => f.filterType === 'LOT_SIZE')?.stepSize || 0.00001)
                };
                
                allPairs.push(pair);
                allQuoteAssets.add(symbol.quoteAsset);
                baseAssetsSet.add(symbol.baseAsset);
                
                // Store mappings for fast lookups
                await redis.hset(REDIS_KEYS.SYMBOL_TO_BASE, pair.symbol, pair.base);
                await redis.hset(REDIS_KEYS.SYMBOL_TO_QUOTE, pair.symbol, pair.quote);
            }
        }
        
        // Sort by quote asset for organized storage
        const pairsByQuote = new Map();
        for (const pair of allPairs) {
            if (!pairsByQuote.has(pair.quote)) {
                pairsByQuote.set(pair.quote, []);
            }
            pairsByQuote.get(pair.quote).push(pair);
        }
        
        // Store ALL pairs
        await redis.set(REDIS_KEYS.ALL_PAIRS, JSON.stringify(allPairs));
        await redis.set(REDIS_KEYS.ALL_SYMBOLS, JSON.stringify(allPairs.map(p => p.symbol)));
        await redis.set(REDIS_KEYS.QUOTE_ASSETS, JSON.stringify(Array.from(allQuoteAssets)));
        
        // Store pairs by quote for fast filtering
        for (const [quote, pairs] of pairsByQuote) {
            await redis.set(REDIS_KEYS.ALL_PAIRS_BY_QUOTE(quote), JSON.stringify(pairs));
        }
        
        console.log(`✅ Loaded ${allPairs.length} total trading pairs`);
        console.log(`📊 Quote assets: ${Array.from(allQuoteAssets).length} different quotes`);
        console.log(`🪙 Base assets: ${baseAssetsSet.size} different coins`);
        
        return { pairs: allPairs, quoteAssets: Array.from(allQuoteAssets) };
        ...
        if (cached) {
            ...
            console.log(`📦 Using cached pairs: ${pairs.length} pairs`);
            return { pairs, quoteAssets: quoteAssets ? JSON.parse(quoteAssets) : ['USDT'] };
        }
        
        throw new Error('Unable to fetch trading pairs and no cache available');
    }
}

// =============================================
// BATCH SUBSCRIPTION - OPTIMIZED WEB SOCKET
// =============================================

async function subscribeToSymbolBatch(symbols) {
    if (!binanceCombinedWs || binanceCombinedWs.readyState !== WebSocket.OPEN) {
        return false;
    }
    ...
    for (const symbol of symbols) {
        if (!subscribedSymbols.has(symbol)) {
            ...
            // Add all required streams for this symbol
            streams.push(`${symbolLower}@ticker`);
            streams.push(`${symbolLower}@depth20@100ms`);
            streams.push(`${symbolLower}@trade`);
            
            for (const interval of INTERVALS) {
                streams.push(`${symbolLower}@kline_${BINANCE_INTERVAL_MAP[interval]}`);
            }
    ...
    // Use combined stream subscription
    const subscribeMsg = {
        method: 'SUBSCRIBE',
        params: streams,
        id: Date.now()
    ...
    try {
        binanceCombinedWs.send(JSON.stringify(subscribeMsg));
        console.log(`📡 Subscribed to ${streams.length} streams for ${symbols.length} symbols`);
        return true;
    } catch (err) {
...
async function subscribeToAllPairs() {
    const { pairs } = await fetchAllBinancePairs();
    const allSymbols = pairs.map(p => p.symbol);
    
    console.log(`🚀 Starting subscription to ${allSymbols.length} symbols...`);
    
    // Subscribe in batches for better performance
    for (let i = 0; i < allSymbols.length; i += AGGREGATOR_CONFIG.BATCH_SUBSCRIBE_SIZE) {
        const batch = allSymbols.slice(i, i + AGGREGATOR_CONFIG.BATCH_SUBSCRIBE_SIZE);
        await subscribeToSymbolBatch(batch);
        
        // Small delay between batches to avoid overwhelming the connection
        await new Promise(resolve => setTimeout(resolve, AGGREGATOR_CONFIG.SUBSCRIBE_DELAY_MS));
        
        const progress = Math.min(100, Math.round((i + batch.length) / allSymbols.length * 100));
        console.log(`📡 Subscription progress: ${progress}% (${i + batch.length}/${allSymbols.length})`);
    }
    
    console.log(`✅ Successfully subscribed to ${subscribedSymbols.size} symbols`);
    await redis.set(REDIS_KEYS.ACTIVE_SYMBOLS, JSON.stringify(Array.from(subscribedSymbols)));
    
    // Update aggregator status
    await redis.hset(REDIS_KEYS.AGGREGATOR_STATUS, 'subscribed_symbols', subscribedSymbols.size);
    await redis.hset(REDIS_KEYS.AGGREGATOR_STATUS, 'last_full_subscription', Date.now());
}

// =============================================
// ENHANCED WEB SOCKET CONNECTION
// =============================================

function connectBinanceWebSocket() {
    if (binanceCombinedWs) {
        try { binanceCombinedWs.close(); } catch(e) {}
    }
    
    console.log('🔌 Connecting to Binance Combined WebSocket...');
    binanceCombinedWs = new WebSocket(AGGREGATOR_CONFIG.BINANCE_WS_COMBINED);
    
    binanceCombinedWs.on('open', async () => {
        console.log('✅ Binance Combined WebSocket connected');
        wsReconnectAttempts = 0;
        
        // Update status
        await redis.set(REDIS_KEYS.WEBSOCKET_STATUS, JSON.stringify({
            status: 'connected',
    ...
    binanceCombinedWs.on('message', async (data) => {
        try {
    ...
    binanceCombinedWs.on('error', (err) => {
        console.error('Binance WebSocket error:', err);
        aggregatorEvents.emit('error', err);
    });
    
    binanceCombinedWs.on('close', () => {
        console.log('Binance WebSocket closed, reconnecting...');
        aggregatorEvents.emit('disconnected');
        
        const delay = Math.min(
            AGGREGATOR_CONFIG.INITIAL_RECONNECT_DELAY * Math.pow(2, wsReconnectAttempts),
            AGGREGATOR_CONFIG.MAX_RECONNECT_DELAY
        );
...
function startHeartbeat() {
    ...
    healthCheckInterval = setInterval(async () => {
        if (binanceCombinedWs && binanceCombinedWs.readyState === WebSocket.OPEN) {
            ...
            try {
                binanceCombinedWs.send(JSON.stringify({ method: 'PING', id: Date.now() }));
                await redis.hset(REDIS_KEYS.AGGREGATOR_STATUS, 'last_heartbeat', Date.now());
            } catch (err) {
                console.error('Heartbeat error:', err);
            }
        } else {
            console.warn('WebSocket not open, attempting reconnect...');
            if (binanceCombinedWs) {
                binanceCombinedWs.close();
            }
            connectBinanceWebSocket();
        }
    }, AGGREGATOR_CONFIG.HEARTBEAT_INTERVAL);
}

// =============================================
// ENHANCED STREAM PROCESSING WITH VALIDATION
// =============================================
...
async function processStreamMessage(message) {
    ...
    // Sequence check for out-of-order messages
    ...
    if (currentSeq <= lastSeq) {
        return; // Skip stale/out-of-order message
    }
    ...
    if (latency > AGGREGATOR_CONFIG.STALE_THRESHOLD_MS) {
        console.debug(`Stale update for ${symbol}@${channel}: ${latency}ms delay`);
        // Still process but log for monitoring
    }
    
    lastUpdateTimeMap.set(`${symbol}:${channel}`, now);
    await redis.hset(REDIS_KEYS.LAST_UPDATE(symbol), channel, now);
...
async function processTickerUpdate(symbol, data) {
    ...
    pipeline.set(REDIS_KEYS.TICKER(symbol), JSON.stringify(tickerData));
    pipeline.set(REDIS_KEYS.LAST_PRICE(symbol), JSON.stringify({
        price: tickerData.lastPrice,
        timestamp: Date.now()
    }));
    pipeline.set(REDIS_KEYS.PRICE_CHANGE(symbol), JSON.stringify({
        change: tickerData.priceChange,
        changePercent: tickerData.priceChangePercent,
        timestamp: Date.now()
    }));
    pipeline.set(REDIS_KEYS.VOLUME_24H(symbol), JSON.stringify({
    ...
    pipeline.hset(REDIS_KEYS.PRICE_INDEX, symbol, tickerData.lastPrice);
    
    await pipeline.exec();
    
    // Broadcast for real-time updates
    if (AGGREGATOR_CONFIG.PRICE_UPDATE_BROADCAST) {
        const pubSubData = {
            type: 'ticker',
            symbol: symbol,
            price: tickerData.lastPrice,
...
async function processOrderBookUpdate(symbol, data) {
    // Parse bids and asks
    const bids = (data.bids || []).slice(0, AGGREGATOR_CONFIG.MAX_ORDER_BOOK_DEPTH)
        .map(b => [parseFloat(b[0]), parseFloat(b[1])]);
    const asks = (data.asks || []).slice(0, AGGREGATOR_CONFIG.MAX_ORDER_BOOK_DEPTH)
    ...
    // Store main orderbook
    await redis.set(REDIS_KEYS.ORDERBOOK(symbol), JSON.stringify(orderbookData));
    
    // Also store different depth levels for quick access
    const depthLevels = [10, 20, 50, 100];
    for (const depth of depthLevels) {
        ...
        await redis.set(REDIS_KEYS.ORDERBOOK_DEPTH(symbol, depth), JSON.stringify(depthData));
    }
    ...
    // Store spread info
    await redis.hset(`market:spread:${symbol}`, {
        bid: bestBid,
        ask: bestAsk,
        spread: spread,
...
async function processTradeUpdate(symbol, data) {
    ...
    const pipeline = redis.pipeline();
    pipeline.lpush(tradesKey, JSON.stringify(tradeData));
    pipeline.ltrim(tradesKey, 0, AGGREGATOR_CONFIG.MAX_RECENT_TRADES - 1);
    await pipeline.exec();
...
async function processKlineUpdate(symbol, data) {
    ...
    if (candleData.isClosed) {
        // Add closed candle to sorted set
        await redis.zadd(candlesKey, candleData.openTime, JSON.stringify(candleData));
        // Keep only last N candles
        await redis.zremrangebyrank(candlesKey, 0, -AGGREGATOR_CONFIG.CANDLE_LIMIT_PER_SYMBOL - 1);
    } else {
    ...
    // Set expiry for candle data (7 days)
    await redis.expire(candlesKey, 604800);
...
// =============================================
// MARKET STATISTICS - REAL-TIME AGGREGATION
// =============================================
...
async function updateMarketStats() {
    try {
        ...
        for (const pair of allPairs.slice(0, 500)) {
            const tickerData = await redis.get(REDIS_KEYS.TICKER(pair.symbol));
            if (tickerData) {
        ...
        const highestVolume = [...tickers]
            .sort((a, b) => b.quoteVolume - a.quoteVolume)
            .slice(0, 20);
        
        // Store statistics
        await redis.set(REDIS_KEYS.TOP_GAINERS, JSON.stringify(gainers));
        await redis.set(REDIS_KEYS.TOP_LOSERS, JSON.stringify(losers));
        await redis.set(REDIS_KEYS.HIGHEST_VOLUME, JSON.stringify(highestVolume));
        
        // Market summary
        const totalVolume = tickers.reduce((sum, t) => sum + (t.quoteVolume || 0), 0);
        const avgChange = tickers.reduce((sum, t) => sum + t.changePercent, 0) / (tickers.length || 1);
        
        await redis.set(REDIS_KEYS.MARKET_SUMMARY, JSON.stringify({
        ...
        console.error('Error updating market stats:', err);
    }
}
...
async function startHealthMonitoring() {
    setInterval(async () => {
        ...
        const status = {
            ...
            lastUpdate: await redis.hgetall(REDIS_KEYS.AGGREGATOR_STATUS),
            uptime: process.uptime(),
        ...
        for (const symbol of subscribedSymbols) {
            const lastUpdate = await redis.hget(REDIS_KEYS.LAST_UPDATE(symbol), 'ticker');
            if (lastUpdate && (now - parseInt(lastUpdate)) > 10000) {
                console.warn(`⚠️ No update for ${symbol} in ${now - parseInt(lastUpdate)}ms`);
            }
        }
...
async function getQuoteAssetsFromRedis() {
    ...
    return ['USDT', 'USDC', 'BTC', 'ETH', 'BNB', 'BUSD', 'EUR', 'GBP', 'TRY', 'RUB', 'BRL', 'DAI'];
}

async function getTicker(symbol) {
    const data = await redis.get(REDIS_KEYS.TICKER(symbol.toUpperCase()));
    return data ? JSON.parse(data) : null;
}

async function getOrderBook(symbol, depth = 20) {
    const key = depth === 20 ? REDIS_KEYS.ORDERBOOK(symbol.toUpperCase()) : REDIS_KEYS.ORDERBOOK_DEPTH(symbol.toUpperCase(), depth);
    const data = await redis.get(key);
    return data ? JSON.parse(data) : null;
}

async function getCandles(symbol, interval, limit = 100) {
    const key = REDIS_KEYS.CANDLES(symbol.toUpperCase(), interval);
    const candles = await redis.zrevrange(key, 0, limit - 1);
    return candles.map(c => JSON.parse(c)).reverse();
}

async function getRecentTrades(symbol, limit = 50) {
    const key = REDIS_KEYS.TRADES(symbol.toUpperCase());
    const trades = await redis.lrange(key, 0, limit - 1);
    return trades.map(t => JSON.parse(t));
}

async function getPairsByQuote(quote) {
    const cached = await redis.get(REDIS_KEYS.ALL_PAIRS_BY_QUOTE(quote));
    if (cached) {
        return JSON.parse(cached);
    }
    const allPairs = await getMarketPairsFromRedis();
    return allPairs.filter(p => p.quote === quote);
}
...
async function initializePriceAggregator() {
    console.log('🚀 Starting Enhanced Price Aggregator v2.0...');
    console.log('=============================================');
    
    // Load all pairs
    const { pairs, quoteAssets } = await fetchAllBinancePairs();
    console.log(`📊 Loaded ${pairs.length} trading pairs`);
    console.log(`📈 Quote assets: ${quoteAssets.join(', ')}`);
    
    // Preload initial prices for all pairs
    console.log('💰 Preloading initial prices...');
    const batchSize = 50;
    let preloaded = 0;
    ...
    for (let i = 0; i < pairs.length; i += batchSize) {
        ...
        const promises = batch.map(async (pair) => {
            const priceKey = REDIS_KEYS.LAST_PRICE(pair.symbol);
            const existing = await redis.get(priceKey);
            if (!existing) {
                try {
                    const response = await axios.get(`https://api.binance.com/api/v3/ticker/price?symbol=${pair.symbol}`, { timeout: 2000 });
                    if (response.data && response.data.price) {
                    ...
                    // Silent fail, WebSocket will populate
                }
            }
        });
        await Promise.all(promises);
        if (i % 500 === 0) {
            console.log(`📊 Preload progress: ${i}/${pairs.length}`);
        }
    }
    console.log(`✅ Preloaded ${preloaded} initial prices`);
    
    // Connect WebSocket
    connectBinanceWebSocket();
    
    // Start statistics updater
    statsUpdateInterval = setInterval(updateMarketStats, 30000);
    
    // Start health monitoring
    startHealthMonitoring();
    
    // Set aggregator ready flag
    await redis.set('aggregator:ready', 'true');
    await redis.hset(REDIS_KEYS.AGGREGATOR_STATUS, 'initialized_at', Date.now());
    await redis.hset(REDIS_KEYS.AGGREGATOR_STATUS, 'status', 'running');
    
    console.log('=============================================');
    console.log('✅ Price Aggregator is HOT and READY!');
    console.log(`📡 All ${pairs.length} pairs available in Redis`);
    console.log(`⚡ Millisecond response times guaranteed`);
    console.log('=============================================');
    
    // Export metrics for monitoring
    setInterval(() => {
        const memory = process.memoryUsage();
        console.log(`📊 Aggregator Stats - Subscribed: ${subscribedSymbols.size}, Memory: ${Math.round(memory.rss / 1024 / 1024)}MB`);
    }, 60000);
}
...
process.on('SIGTERM', async () => {
    ...
    if (binanceCombinedWs) binanceCombinedWs.close();
    await redis.set('aggregator:ready', 'false');
    process.exit(0);
});
...
module.exports = {
    getMarketPairsFromRedis,
    getQuoteAssetsFromRedis,
    getTicker,
    getOrderBook,
    getCandles,
    getRecentTrades,
...
app.post('/api/investments/:id/complete', protect, async (req, res) => {
  try {
    ...
    // Get current BTC price from API using the aggregator
    const currentBTCPrice = await getRealTimeBitcoinPrice();
    
    // Calculate total return in BTC using the aggregator
    // Convert the expected USD return to BTC using current price
    ...
    try {
      ...
      // =============================================
      // SEND SISTER EMAIL FOR INVESTMENT MATURATION
      // Using direct email sending to match deposit_approved style
      // =============================================
      try {
        const getCryptoLogoUrl = (asset) => {
            ...
            <div style="padding: 30px; background: #FFFFFF;">
              ...
              <div style="background: #F5F5F5; padding: 20px; border-radius: 12px; margin: 20px 0;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr style="border-top: 1px solid #E2E8F0;">
                    <td style="padding: 8px 0;"><strong>New Matured Wallet Balance:</strong><tr>
      ...
      res.status(200).json({
        ...
        data: {
          investment: {
            ...
            amountReturnedUSD: expectedReturnUSD,
            profitBTC: totalReturnBTC - investment.amountBTC,
            profitUSD: expectedReturnUSD - investment.amount,
            btcPriceAtStart: investment.btcPriceAtInvestment,
            btcPriceAtCompletion: currentBTCPrice
...
// =============================================
// TRADING ENDPOINTS - ENTERPRISE GRADE
// All pairs come from Redis (Binance aggregator)
// No hardcoded pairs - fully dynamic
// =============================================

// =============================================
// GET /api/market/all-pairs - All available trading pairs from Redis
// =============================================
app.get('/api/market/all-pairs', async (req, res) => {
  try {
    // Get all pairs from Redis (populated by price aggregator)
    const allPairsRaw = await redis.get(REDIS_KEYS.ALL_PAIRS);
    const quoteAssetsRaw = await redis.get(REDIS_KEYS.QUOTE_ASSETS);
    
    let allPairs = [];
    let quoteAssets = ['USDT'];
    
    if (allPairsRaw) {
      allPairs = JSON.parse(allPairsRaw);
    } else {
      ...
      const usdtPairs = symbols.filter(s => s.quoteAsset === 'USDT' && s.status === 'TRADING');
      const usdcPairs = symbols.filter(s => s.quoteAsset === 'USDC' && s.status === 'TRADING');
      const btcPairs = symbols.filter(s => s.quoteAsset === 'BTC' && s.status === 'TRADING');
      const ethPairs = symbols.filter(s => s.quoteAsset === 'ETH' && s.status === 'TRADING');
      const bnbPairs = symbols.filter(s => s.quoteAsset === 'BNB' && s.status === 'TRADING');
      
      allPairs = [
        ...usdtPairs.map(p => ({ symbol: p.symbol, base: p.baseAsset, quote: 'USDT', status: 'active' })),
        ...usdcPairs.map(p => ({ symbol: p.symbol, base: p.baseAsset, quote: 'USDC', status: 'active' })),
        ...btcPairs.map(p => ({ symbol: p.symbol, base: p.baseAsset, quote: 'BTC', status: 'active' })),
        ...ethPairs.map(p => ({ symbol: p.symbol, base: p.baseAsset, quote: 'ETH', status: 'active' })),
        ...bnbPairs.map(p => ({ symbol: p.symbol, base: p.baseAsset, quote: 'BNB', status: 'active' }))
      ];
      
      quoteAssets = ['USDT', 'USDC', 'BTC', 'ETH', 'BNB'];
      
      // Cache for future requests
      await redis.setex(REDIS_KEYS.ALL_PAIRS, 3600, JSON.stringify(allPairs));
      await redis.setex(REDIS_KEYS.QUOTE_ASSETS, 3600, JSON.stringify(quoteAssets));
    }
    
    // Enhance each pair with current price from Redis
    const enhancedPairs = [];
    for (const pair of allPairs.slice(0, 200)) {
      ...
      if (tickerRaw) {
        ...
        enhancedPairs.push({ ...pair, price: 0, priceChangePercent: 0 });
      }
    }
...
// =============================================
// GET /api/market/pairs - Get filtered pairs for market table
// =============================================
app.get('/api/market/pairs', async (req, res) => {
  try {
    const { quote = 'USDT', limit = 100, search = '' } = req.query;
    
    const allPairsRaw = await redis.get(REDIS_KEYS.ALL_PAIRS);
    if (!allPairsRaw) {
      return res.status(503).json({
        status: 'error',
        message: 'Market data not ready. Please try again.'
      });
    }
    
    let allPairs = JSON.parse(allPairsRaw);
    
    // Filter by quote asset
    let filteredPairs = allPairs.filter(p => p.quote === quote);
    
    // Apply search filter if provided
    if (search) {
      const searchLower = search.toLowerCase();
      filteredPairs = filteredPairs.filter(p => 
        p.base.toLowerCase().includes(searchLower) || 
        p.symbol.toLowerCase().includes(searchLower)
      );
    }
    
    // Limit results
    filteredPairs = filteredPairs.slice(0, parseInt(limit));
    ...
    for (const pair of filteredPairs) {
      ...
      enhancedPairs.push({
        ...
        high24h: high,
        low24h: low,
        logoUrl: `https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/128/color/${pair.base.toLowerCase()}.png`
      });
    }
...
// =============================================
// GET /api/market/orderbook - Get order book for a symbol
// =============================================
app.get('/api/market/orderbook', async (req, res) => {
  try {
    ...
    // Try Redis first (from aggregator)
    const orderbookKey = REDIS_KEYS.ORDERBOOK(symbolUpper);
    ...
    if (orderbookRaw) {
      ...
      return res.status(200).json({
        status: 'success',
        bids: orderbook.bids.slice(0, parseInt(limit)),
        asks: orderbook.asks.slice(0, parseInt(limit)),
    ...
    const response = await axios.get(`https://api.binance.com/api/v3/depth?symbol=${symbolUpper}&limit=${limit}`, { timeout: 5000 });
...
// =============================================
// GET /api/market/ticker/24hr - Get 24hr ticker for a symbol
// =============================================
app.get('/api/market/ticker/24hr', async (req, res) => {
  try {
    ...
    if (!symbol) {
      // Return all top tickers if no symbol provided
      const allPairsRaw = await redis.get(REDIS_KEYS.ALL_PAIRS);
      if (allPairsRaw) {
    ...
    // Try Redis first
    const tickerKey = REDIS_KEYS.TICKER(symbolUpper);
    const tickerRaw = await redis.get(tickerKey);
    ...
    // Fallback to Binance API
    const response = await axios.get(`https://api.binance.com/api/v3/ticker/24hr?symbol=${symbolUpper}`, { timeout: 5000 });
...
// =============================================
// GET /api/market/candles - Get candlestick data for chart
// =============================================
app.get('/api/market/candles', async (req, res) => {
  try {
    ...
    // Try Redis first (from aggregator)
    const candlesKey = REDIS_KEYS.CANDLES(symbolUpper, binanceInterval);
...
// =============================================
// GET /api/market/trades - Get recent trades for a symbol
// =============================================
app.get('/api/market/trades', async (req, res) => {
  try {
    ...
    // Try Redis first
    const tradesKey = REDIS_KEYS.TRADES(symbolUpper);
...
// =============================================
// GET /api/trading/user-limits - Get user's trading limits and balances
// =============================================
app.get('/api/trading/user-limits', protect, async (req, res) => {
  try {
    ...
    // Find the pair in Redis
    const allPairsRaw = await redis.get(REDIS_KEYS.ALL_PAIRS);
    if (allPairsRaw) {
      const allPairs = JSON.parse(allPairsRaw);
      const pair = allPairs.find(p => p.symbol === symbol.toUpperCase());
      if (pair) {
        baseAsset = pair.base;
    ...
    if (!baseAsset || !quoteAsset) {
      // Default to BTC/USDT pattern
      baseAsset = symbol.replace(/USDT$|USDC$|BTC$|ETH$/, '');
      quoteAsset = symbol.slice(baseAsset.length);
    }
    ...
    if (tickerRaw) {
      ...
      // Fallback to API
      try {
        const response = await axios.get(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol.toUpperCase()}`, { timeout: 3000 });
    ...
    if (side === 'buy') {
      // For buy: need quote asset balance (USDT, USDC, etc.)
      const quoteLower = quoteAsset.toLowerCase();
      const mainBalance = user.balances.main.get(quoteLower) || 0;
      const maturedBalance = user.balances.matured.get(quoteLower) || 0;
      availableBalance = mainBalance + maturedBalance;
      
      if (currentPrice > 0) {
        maxAmount = availableBalance / currentPrice;
      } else {
        maxAmount = availableBalance;
      }
    } else {
      // For sell: need base asset balance
...
// =============================================
// POST /api/trading/orders/buy - Place a buy order
// =============================================
app.post('/api/trading/orders/buy', protect, async (req, res) => {
  try {
    ...
    const mainBalance = user.balances.main.get(quoteLower) || 0;
    const maturedBalance = user.balances.matured.get(quoteLower) || 0;
    const availableQuoteBalance = mainBalance + maturedBalance;
    
    if (totalCost > availableQuoteBalance) {
      return res.status(400).json({
        status: 'fail',
        message: `Insufficient ${quoteAsset} balance. Required: ${totalCost.toFixed(2)}, Available: ${availableQuoteBalance.toFixed(2)}`
      });
    }
    ...
    if (type === 'market') {
      ...
      const newMainBalance = mainBalance - totalCost;
      const newMaturedBalance = maturedBalance - (totalCost > mainBalance ? totalCost - mainBalance : 0);
      
      if (newMainBalance <= 0) {
        user.balances.main.delete(quoteLower);
      } else {
        user.balances.main.set(quoteLower, newMainBalance);
      }
      
      if (newMaturedBalance <= 0) {
        user.balances.matured.delete(quoteLower);
      } else if (newMaturedBalance > 0 && newMaturedBalance < maturedBalance) {
        user.balances.matured.set(quoteLower, newMaturedBalance);
      }
      
      // Add base asset to main wallet
      const baseLower = baseAsset.toLowerCase();
      const currentBaseBalance = user.balances.main.get(baseLower) || 0;
      user.balances.main.set(baseLower, currentBaseBalance + amount);
    ...
    const order = await Order.create({
      ...
      quoteOrderQty: totalCost,
      status: orderStatus,
      timeInForce: 'GTC',
      fee: type === 'market' ? totalCost * 0.001 : 0,
      feeAsset: quoteAsset,
      total: totalCost,
      avgPrice: currentPrice
    });
    
    // If market order, also create a trade record
    if (type === 'market') {
      const tradeId = `TRD-${Date.now()}-${Math.random().toString(36).substr(2, 8).toUpperCase()}`;
      ...
      await PlatformRevenue.create({
        source: 'taker_fee',
        amount: totalCost * 0.001,
        currency: quoteAsset,
        orderId: orderId,
        userId: userId,
        symbol: symbol.toUpperCase(),
        description: `Market buy order fee for ${symbol}`,
        metadata: {
          orderType: type,
          orderSide: 'buy',
          amount: amount,
    ...
    if (io && type === 'market') {
      ...
      io.to(`user_${userId}`).emit('order_executed', {
        orderId: orderId,
        side: 'buy',
        amount: amount,
        price: currentPrice,
...
// =============================================
// POST /api/trading/orders/sell - Place a sell order
// =============================================
app.post('/api/trading/orders/sell', protect, async (req, res) => {
  try {
    ...
    const mainBalance = user.balances.main.get(baseLower) || 0;
    const maturedBalance = user.balances.matured.get(baseLower) || 0;
    const availableBaseBalance = mainBalance + maturedBalance;
    
    if (amount > availableBaseBalance) {
      return res.status(400).json({
        status: 'fail',
        message: `Insufficient ${baseAsset} balance. Required: ${amount.toFixed(8)}, Available: ${availableBaseBalance.toFixed(8)}`
      });
    }
    ...
    if (type === 'market') {
      ...
      const newMaturedBalance = maturedBalance - (amount > mainBalance ? amount - mainBalance : 0);
      
      if (newMainBalance <= 0) {
        user.balances.main.delete(baseLower);
      } else {
        user.balances.main.set(baseLower, newMainBalance);
      }
      
      if (newMaturedBalance <= 0) {
        user.balances.matured.delete(baseLower);
      } else if (newMaturedBalance > 0 && newMaturedBalance < maturedBalance) {
        user.balances.matured.set(baseLower, newMaturedBalance);
      }
      
      // Add quote asset to main wallet
      const quoteLower = quoteAsset.toLowerCase();
      const currentQuoteBalance = user.balances.main.get(quoteLower) || 0;
      const fee = totalValue * 0.001;
      const netAmount = totalValue - fee;
      user.balances.main.set(quoteLower, currentQuoteBalance + netAmount);
      
      // Update USD equivalent
      const currentUSD = user.balances.main.get('usd') || 0;
      user.balances.main.set('usd', currentUSD + netAmount);
    ...
    const order = await Order.create({
      ...
      executedQty: executedQty,
      remainingQty: remainingQty,
      quoteOrderQty: totalValue,
      status: orderStatus,
      timeInForce: 'GTC',
      fee: type === 'market' ? totalValue * 0.001 : 0,
      feeAsset: quoteAsset,
      total: totalValue,
      avgPrice: currentPrice
    });
    ...
    if (type === 'market') {
      ...
      await PlatformRevenue.create({
        source: 'taker_fee',
        amount: totalValue * 0.001,
        currency: quoteAsset,
        orderId: orderId,
        userId: userId,
        symbol: symbol.toUpperCase(),
        description: `Market sell order fee for ${symbol}`,
        metadata: {
          orderType: type,
          orderSide: 'sell',
          amount: amount,
    ...
    // Log activity    await logActivity(
      'sell_created',
      'Order',
      order._id,
      userId,
...
// =============================================
// POST /api/trading/orders/cancel - Cancel an order
// =============================================
...
// =============================================
// POST /api/trading/orders/cancel-all - Cancel all open orders for a symbol
// =============================================
...
app.get('/api/trading/positions', protect, async (req, res) => {
  try {
    ...
    for (const pos of positions) {
      const tickerKey = REDIS_KEYS.TICKER(pos.symbol);
      const tickerRaw = await redis.get(tickerKey);
      if (tickerRaw) {
        const ticker = JSON.parse(tickerRaw);
...
app.post('/api/trading/positions/close', protect, async (req, res) => {
  try {
    ...
    // Get current price
    const tickerKey = REDIS_KEYS.TICKER(position.symbol);
    const tickerRaw = await redis.get(tickerKey);
    let currentPrice = 0;
    ...
    if (tickerRaw) {
      ...
      try {
        const response = await axios.get(`https://api.binance.com/api/v3/ticker/price?symbol=${position.symbol}`, { timeout: 3000 });
        currentPrice = parseFloat(response.data.price);
      } catch (err) {
        return res.status(503).json({
          status: 'fail',
    ...
    const user = await User.findById(userId);
    if (user) {
      const baseAsset = position.symbol.replace(/USDT$|USDC$|BTC$|ETH$/, '');
      ...
      // Return margin plus PnL as quote asset
      const currentQuoteBalance = user.balances.main.get(quoteLower) || 0;
      const newBalance = currentQuoteBalance + position.margin + realizedPnL;
      user.balances.main.set(quoteLower, newBalance);
      
      // Update USD equivalent
      const currentUSD = user.balances.main.get('usd') || 0;
      user.balances.main.set('usd', currentUSD + position.margin + realizedPnL);
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(`user_${userId}`).emit('position_closed', {
...
app.post('/api/trading/orders/tpsl', protect, async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 'success',
      message: 'Take profit / Stop loss set successfully',
      data: {
    ...
    res.status(500).json({
      status: 'error',
      message: 'Failed to set take profit/stop loss'
    });
  }
});

// =============================================
// GET /api/asset/logos - Get logo URLs for assets
// =============================================
app.get('/api/asset/logos', async (req, res) => {
  try {
    ...
    if (base) {
      logos.baseLogo = `https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/128/color/${base.toLowerCase()}.png`;
    }
    if (quote) {
      logos.quoteLogo = `https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/128/color/${quote.toLowerCase()}.png`;
    }
...
// =============================================
// GET /api/asset/info - Get asset information
// =============================================
app.get('/api/asset/info', async (req, res) => {
  try {
    ...
    // Try Redis first
    const assetInfoKey = REDIS_KEYS.ASSET_INFO(symbol.toLowerCase());
...
// =============================================
// GET /api/asset/extra - Get extra asset info (tags, networks)
// =============================================
app.get('/api/asset/extra', async (req, res) => {
  try {
    ...
    // Try Redis
    const extraKey = `asset:extra:${symbolUpper}`;
    const cachedExtra = await redis.get(extraKey);
    
    if (cachedExtra) {
      return res.status(200).json(JSON.parse(cachedExtra));
    }
    
    // Default tags based on known assets
    const tagsMap = {
      'BTC': ['POW', 'Store of Value', 'Digital Gold', 'Payments'],
      'ETH': ['Smart Contracts', 'dApps', 'DeFi', 'NFT'],
      'BNB': ['Exchange Token', 'Smart Chain', 'BSC'],
      'SOL': ['High Performance', 'DeFi', 'NFT'],
      'USDT': ['Stablecoin', 'Payments'],
      'USDC': ['Stablecoin', 'Regulated'],
      'XRP': ['Payments', 'Enterprise'],
      'DOGE': ['Meme', 'Payments'],
      'ADA': ['Smart Contracts', 'Research'],
      'AVAX': ['Subnets', 'EVM Compatible'],
      'DOT': ['Parachains', 'Interoperability'],
      'LINK': ['Oracle', 'DeFi'],
      'MATIC': ['Layer 2', 'Scaling'],
      'LTC': ['Payments', 'Silver to Bitcoin\'s Gold'],
      'SHIB': ['Meme', 'Ecosystem'],
      'TRX': ['dApps', 'High Throughput'],
      'ATOM': ['Interoperability', 'IBC'],
      'NEAR': ['Sharding', 'Usability'],
      'ALGO': ['Pure PoS', 'Fast Finality'],
      'VET': ['Supply Chain', 'Enterprise']
    };
    
    const networksMap = {
      'BTC': ['Bitcoin'],
      'ETH': ['Ethereum (ERC-20)'],
      'USDT': ['Ethereum (ERC-20)', 'Tron (TRC-20)', 'Solana', 'BSC'],
      'USDC': ['Ethereum (ERC-20)', 'Solana', 'BSC'],
      'BNB': ['BSC'],
      'SOL': ['Solana'],
      'XRP': ['XRP Ledger'],
      'TRX': ['Tron']
    ...
    const response = {
      tags: tagsMap[symbolUpper] || ['Crypto', 'Digital Asset'],
      networks: networksMap[symbolUpper] || ['Blockchain']
    };
    
    // Cache for 1 hour
    await redis.setex(extraKey, 3600, JSON.stringify(response));
    
    res.status(200).json(response);
    
  } catch (err) {
    console.error('Error fetching extra asset info:', err);
    res.status(200).json({
      tags: ['Crypto'],
      networks: ['Blockchain']
    });
...
app.post('/api/user/chart-settings', protect, async (req, res) => {
  try {
    ...
    const settingsKey = `user:${userId}:chart_settings`;
    await redis.setex(settingsKey, 86400, JSON.stringify(chartSettings));
...
app.post('/api/user/settings', protect, async (req, res) => {
  try {
    ...
    const updateData = {};
    if (orderBookSettings) updateData.orderBookSettings = orderBookSettings;
    if (chartSettings) updateData.chartSettings = chartSettings;
    if (notifications) updateData.notifications = notifications;
    if (defaultLeverage !== undefined) updateData.defaultLeverage = defaultLeverage;
    if (defaultOrderType !== undefined) updateData.defaultOrderType = defaultOrderType;
...
// =============================================
// WEBSOCKET HANDLER FOR SPOT MARKET (already in setupSpotMarketWebSocket)
// The following ensures proper WebSocket endpoints are initialized
// =============================================

// The WebSocket setup functions are already defined and called in httpServer.listen
// They are: setupSpotMarketWebSocket, setupTickerWebSocket, setupMarketWebSocket
// These are initialized when the server starts.

console.log('✅ All trading endpoints registered successfully');
console.log('📊 Market data flows from Binance WebSocket → Redis → REST endpoints');
console.log('💰 User balances use Map structure for crypto assets');
console.log('🎨 Chart markers integrate with Lightweight Charts via backend orders');
