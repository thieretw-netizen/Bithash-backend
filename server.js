...
// POST /api/admin/email-templates - Save new email template
app.post('/api/admin/email-templates', adminProtect, async (req, res) => {
  try {
    ...
    const existingTemplate = await EmailTemplate.findOne({ name: name });
    if (existingTemplate) {
      return res.status(400).json({
        status: 'fail',
  ...
  try {
    ...
    const formattedEmails = emails.map(email => {
      ...
      if (email.recipientCount > 0) {
        const openedCount = email.recipients.filter(r => r.openedAt).length;
        openRate = Math.round((openedCount / email.recipientCount) * 100);
      }
      ...
      return {
        ...
        openRate: openRate,
        status: email.status,
        sentBy: email.sentBy?.name || 'Admin',
        sentAt: email.sentAt,
...
app.post('/api/admin/send-email', adminProtect, async (req, res) => {
  try {
    ...
    if (emailHistory.recipientCount > 0) {
      openRate = Math.round((openedCount / emailHistory.recipientCount) * 100);
    }
...
// GET /api/email/track/:emailId - Email open tracking endpoint
