...
// =============================================
// PRODUCTION PDF GENERATION WITH FULL GRAPHICS & CHARTS
// =============================================
...
// =============================================
// HELPER: Generate Donut Chart as Buffer
// =============================================
async function generateDonutChart(labels, values, width = 400, height = 300) {
  ...
  const colors = ['#10B981', '#F59E0B', '#3B82F6', '#8B5CF6', '#EF4444', '#06B6D4', '#EC4899', '#84CC16'];
  ...
  ctx.font = 'bold 20px "Inter", "Helvetica"';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(`$${total.toLocaleString()}`, centerX, centerY);
  
  ctx.font = '10px "Inter", "Helvetica"';
  ...
  for (let i = 0; i < Math.min(labels.length, 6); i++) {
    ...
    ctx.font = '10px "Inter", "Helvetica"';
    ctx.textAlign = 'left';
    ctx.fillText(`${labels[i]}: $${values[i].toLocaleString()}`, legendX + 18, legendY + 10);
...
// =============================================
// HELPER: Generate Line Chart as Buffer
// =============================================
async function generateLineChart(labels, values, title = 'Transaction Volume', width = 500, height = 300) {
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext('2d');
  ...
  // Draw title
  ctx.fillStyle = '#0B0E11';
  ctx.font = 'bold 14px "Inter", "Helvetica"';
  ctx.textAlign = 'center';
  ...
  if (values.length > 1) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      const y = margin.top + chartHeight - ((values[i] - minValue) / valueRange) * chartHeight;
      if (firstPoint) {
        ctx.moveTo(x, y);
        firstPoint = false;
      } else {
    ...
    ctx.lineTo(margin.left, height - margin.bottom);
    ctx.closePath();
    ctx.fillStyle = 'rgba(247, 166, 0, 0.1)'; // Brand color
    ctx.fill();
    ...
    ctx.strokeStyle = '#F7A600'; // Brand gold color
    ctx.lineWidth = 2.5;
    ctx.stroke();
    ...
    ctx.font = '9px "Inter", "Helvetica"';
    ctx.textAlign = 'center';
    for (let i = 0; i < labels.length; i++) {
      const x = margin.left + i * stepX;
      ctx.fillText(labels[i], x, height - margin.bottom + 20);
    }
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      ctx.fillText(`$${Math.round(value).toLocaleString()}`, margin.left - 10, y + 3);
    }
  }
...
// =============================================
// HELPER: Generate Bar Chart as Buffer
// =============================================
async function generateBarChart(labels, values, title = 'Wallet Distribution', width = 500, height = 300) {
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext('2d');
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    // Value on top of bar
    ctx.fillStyle = '#0B0E11';
    ctx.font = 'bold 10px "Inter", "Helvetica"';
    ctx.textAlign = 'center';
    ctx.fillText(`$${values[i].toLocaleString()}`, x + barWidth / 2, y - 5);
...
// =============================================
// PRODUCTION PDF GENERATION WITH FULL GRAPHICS
// =============================================
async function generateProfessionalPDFStatement(statement, user, chartData) {
  return new Promise(async (resolve, reject) => {
    try {
      const doc = new PDFDocument({ 
        ...
        size: 'A4',
        info: {
          Title: `${statement.statementType.toUpperCase()} Financial Statement - ${user.firstName} ${user.lastName}`,
          Author: '₿itHash Capital',
      ...
      doc.fontSize(28).font('Helvetica-Bold').fillColor('#F7A600');
      doc.text('₿', 50, 45, { align: 'left' });
      doc.fontSize(24).fillColor('#FFFFFF');
      doc.text('itHash Capital', 75, 48);
      
      doc.fontSize(10).fillColor('#B7BDC6');
      doc.text('Institutional Bitcoin Mining', 50, 80);
      doc.text('Where Your Financial Goals Become Reality', 50, 95);
      
      // Statement title
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text(`${statement.statementType.toUpperCase()} FINANCIAL STATEMENT`, 50, 150, { align: 'center' });
      
      doc.fontSize(10).font('Helvetica').fillColor('#6C7480');
      doc.text(`Period: ${new Date(statement.period.startDate).toLocaleDateString()} - ${new Date(statement.period.endDate).toLocaleDateString()}`, 0, 175, { align: 'center' });
      doc.text(`Generated: ${new Date(statement.period.generationDate).toLocaleString()}`, 0, 190, { align: 'center' });
      
      // User info box
      doc.rect(50, 210, 500, 60).fill('#F9FAFB').stroke('#E5E7EB');
      doc.fontSize(12).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text('Account Holder:', 65, 225);
      doc.fontSize(12).font('Helvetica').fillColor('#333333');
      doc.text(`${user.firstName} ${user.lastName}`, 180, 225);
      doc.text(`Email: ${user.email}`, 65, 245);
      
      // =============================================
      // PERFORMANCE SUMMARY CARD
      // =============================================
      doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text('Performance Summary', 50, 300);
      
      doc.rect(50, 315, 240, 100).fill('#F0FDF4').stroke('#D1FAE5');
      doc.rect(310, 315, 240, 100).fill('#FEF2F2').stroke('#FEE2E2');
      doc.rect(50, 430, 240, 100).fill('#EFF6FF').stroke('#BFDBFE');
      doc.rect(310, 430, 240, 100).fill('#FEF3C7').stroke('#FDE68A');
      
      // Opening Balance
      doc.fontSize(10).font('Helvetica').fillColor('#065F46');
      doc.text('OPENING BALANCE', 60, 330);
      doc.fontSize(20).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text(`$${statement.openingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 60, 350);
      
      // Closing Balance
      doc.fontSize(10).font('Helvetica').fillColor('#991B1B');
      doc.text('CLOSING BALANCE', 320, 330);
      doc.fontSize(20).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text(`$${statement.closingBalances.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 320, 350);
      
      // Net Change
      doc.fontSize(10).font('Helvetica').fillColor('#1E40AF');
      doc.text('NET CHANGE', 60, 445);
      const changeColor = statement.netChangeUSD >= 0 ? '#10B981' : '#EF4444';
      doc.fontSize(20).font('Helvetica-Bold').fillColor(changeColor);
      doc.text(`${statement.netChangeUSD >= 0 ? '+' : ''}$${Math.abs(statement.netChangeUSD).toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 60, 465);
      
      // Net Profit/Loss
      doc.fontSize(10).font('Helvetica').fillColor('#92400E');
      doc.text('NET PROFIT / LOSS', 320, 445);
      const profitColor = statement.summary.netProfitUSD >= 0 ? '#10B981' : '#EF4444';
      doc.fontSize(20).font('Helvetica-Bold').fillColor(profitColor);
      doc.text(`${statement.summary.netProfitUSD >= 0 ? '+' : ''}$${Math.abs(statement.summary.netProfitUSD).toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 320, 465);
      doc.fontSize(10).font('Helvetica').fillColor('#6B7280');
      doc.text(`(${statement.summary.roiPercentage >= 0 ? '+' : ''}${statement.summary.roiPercentage.toFixed(2)}% ROI)`, 320, 490);
      
      // =============================================
      // WALLET BREAKDOWN CHART
      // =============================================
      doc.addPage();
      
      doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text('Wallet Breakdown', 50, 50);
      
      // Generate donut chart
      const walletLabels = ['Main Wallet', 'Active Mining', 'Matured Wallet'];
      const walletValues = [
        statement.closingBalances.mainWalletUSD,
        statement.closingBalances.activeWalletUSD,
        statement.closingBalances.maturedWalletUSD
      ];
      
      const donutChartBuffer = await generateDonutChart(walletLabels, walletValues, 350, 300);
      doc.image(donutChartBuffer, 50, 70, { width: 350 });
      
      // Wallet details table
      const walletY = 390;
      doc.fontSize(10).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text('Wallet Details', 50, walletY);
      
      doc.fontSize(9).font('Helvetica');
      const walletRows = [
        ['Main Wallet (Fluctuates)', `$${statement.closingBalances.mainWalletUSD.toLocaleString()}`, 'Crypto holdings that change with market prices'],
        ['Active Mining (Fixed)', `$${statement.closingBalances.activeWalletUSD.toLocaleString()}`, 'Mining contracts - value does not fluctuate'],
        ['Matured Wallet (Fluctuates)', `$${statement.closingBalances.maturedWalletUSD.toLocaleString()}`, 'Completed investments and returns']
      ];
      
      let rowY = walletY + 20;
      for (const row of walletRows) {
        doc.fillColor('#333333');
        doc.text(row[0], 50, rowY);
        doc.fillColor(row[1].startsWith('+') ? '#10B981' : '#0B0E11');
        doc.text(row[1], 220, rowY);
        doc.fillColor('#6B7280');
        doc.text(row[2], 300, rowY, { width: 200 });
        rowY += 22;
      }
      ...
      // =============================================
      if (chartData && chartData.dailyVolume) {
        doc.addPage();
        
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Transaction Activity', 50, 50);
        
        const lineChartBuffer = await generateLineChart(
          chartData.dailyVolume.labels,
          chartData.dailyVolume.values,
          'Daily Transaction Volume (Last 7 Days)',
          500,
          300
        );
        doc.image(lineChartBuffer, 50, 70, { width: 500 });
      }
      
      // =============================================
      // CRYPTO HOLDINGS
      // =============================================
      if (statement.closingBalances.cryptoDetails && statement.closingBalances.cryptoDetails.length > 0) {
        doc.addPage();
        
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Crypto Holdings', 50, 50);
        
        // Generate bar chart for top cryptos
        const topCryptos = statement.closingBalances.cryptoDetails.slice(0, 8);
        const cryptoLabels = topCryptos.map(c => c.asset.toUpperCase());
        const cryptoValues = topCryptos.map(c => c.usdValue);
        
        const barChartBuffer = await generateBarChart(cryptoLabels, cryptoValues, 'Asset Distribution', 500, 300);
        doc.image(barChartBuffer, 50, 70, { width: 500 });
        
        // Detailed holdings table
        let tableY = 390;
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Detailed Holdings', 50, tableY);
        
        // Table headers
        tableY += 15;
        doc.fillColor('#6B7280').fontSize(8);
        doc.text('Asset', 50, tableY);
        doc.text('Amount', 180, tableY);
        doc.text('Current Price', 280, tableY);
        doc.text('USD Value', 380, tableY);
        ...
        for (const crypto of topCryptos) {
          ...
          doc.text(crypto.asset.toUpperCase(), 50, tableY);
          doc.text(crypto.amount.toFixed(8), 180, tableY);
          doc.text(`$${crypto.currentPrice?.toLocaleString() || 'N/A'}`, 280, tableY);
          doc.fillColor('#0B0E11').font('Helvetica-Bold');
          doc.text(`$${crypto.usdValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 380, tableY);
      ...
      doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text('Investment Summary', 50, 50);
      
      // Investment stats boxes
      doc.rect(50, 70, 240, 80).fill('#F0FDF4').stroke('#D1FAE5');
      doc.rect(310, 70, 240, 80).fill('#EFF6FF').stroke('#BFDBFE');
      doc.rect(50, 165, 240, 80).fill('#FEF3C7').stroke('#FDE68A');
      doc.rect(310, 165, 240, 80).fill('#F3E8FF').stroke('#E9D5FF');
      
      doc.fontSize(10).font('Helvetica').fillColor('#065F46');
      doc.text('TOTAL INVESTED', 60, 85);
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text(`$${statement.investments.summary.totalPrincipalInvestedUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 60, 105);
      
      doc.fontSize(10).font('Helvetica').fillColor('#1E40AF');
      doc.text('TOTAL RETURNS', 320, 85);
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#10B981');
      doc.text(`+$${statement.investments.summary.totalReturnsEarnedUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 320, 105);
      
      doc.fontSize(10).font('Helvetica').fillColor('#92400E');
      doc.text('ACTIVE INVESTMENTS', 60, 180);
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#0B0E11');
      doc.text(statement.investments.summary.totalActiveInvestmentsCount.toString(), 60, 200);
      
      doc.fontSize(10).font('Helvetica').fillColor('#6B21A8');
      doc.text('TOTAL PROFIT', 320, 180);
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#10B981');
      doc.text(`$${statement.investments.summary.totalProfitUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 320, 200);
      
      // Active investments table
      if (statement.investments.active && statement.investments.active.length > 0) {
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Active Investments', 50, 270);
        
        let tableY = 290;
        doc.fillColor('#6B7280').fontSize(8);
        doc.text('Plan', 50, tableY);
        doc.text('Principal', 200, tableY);
        doc.text('Expected Return', 300, tableY);
        doc.text('Maturity', 400, tableY);
        
        tableY += 5;
        doc.moveTo(50, tableY).lineTo(550, tableY).stroke();
        
        for (const inv of statement.investments.active.slice(0, 10)) {
          tableY += 20;
          doc.fillColor('#333333').fontSize(9);
          doc.text(inv.planName, 50, tableY);
          doc.text(`$${inv.principalUSD.toLocaleString()}`, 200, tableY);
          doc.text(`$${inv.expectedReturnUSD.toLocaleString()}`, 300, tableY);
          doc.text(new Date(inv.endDate).toLocaleDateString(), 400, tableY);
      ...
      if (statement.transactions.list && statement.transactions.list.length > 0) {
        ...
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Recent Transactions', 50, 50);
        ...
        doc.text('Type', 120, tableY);
        doc.text('Asset', 200, tableY);
        doc.text('Amount', 280, tableY);
        doc.text('Status', 360, tableY);
        doc.text('Reference', 430, tableY);
        
        tableY += 5;
        doc.moveTo(50, tableY).lineTo(550, tableY).stroke();
        
        for (const tx of statement.transactions.list.slice(0, 20)) {
          tableY += 20;
          
          const txAmount = tx.type === 'withdrawal' ? -tx.amountUSD : tx.amountUSD;
          const amountColor = txAmount >= 0 ? '#10B981' : '#EF4444';
          
          doc.fillColor('#333333').fontSize(8);
          doc.text(new Date(tx.createdAt).toLocaleDateString(), 50, tableY);
          doc.text(tx.type, 120, tableY);
          doc.text(tx.asset || 'USD', 200, tableY);
          doc.fillColor(amountColor);
          doc.text(`${txAmount >= 0 ? '+' : ''}$${Math.abs(txAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 280, tableY);
          doc.fillColor(tx.status === 'completed' ? '#10B981' : tx.status === 'pending' ? '#F59E0B' : '#EF4444');
          doc.text(tx.status, 360, tableY);
          doc.fillColor('#6B7280');
          doc.text(tx.reference?.substring(0, 12) || 'N/A', 430, tableY, { width: 100 });
      ...
      // =============================================
      if (statement.trading && (statement.trading.buys.length > 0 || statement.trading.sells.length > 0)) {
        doc.addPage();
        
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Trading Activity', 50, 50);
        
        // Trading stats
        doc.rect(50, 70, 240, 80).fill('#F0FDF4').stroke('#D1FAE5');
        doc.rect(310, 70, 240, 80).fill('#FEF2F2').stroke('#FEE2E2');
        
        doc.fontSize(10).font('Helvetica').fillColor('#065F46');
        doc.text('TOTAL BUYS', 60, 85);
        doc.fontSize(16).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text(`$${statement.trading.summary.totalBuyVolumeUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 60, 105);
        
        doc.fontSize(10).font('Helvetica').fillColor('#991B1B');
        doc.text('TOTAL SELLS', 320, 85);
        doc.fontSize(16).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text(`$${statement.trading.summary.totalSellVolumeUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 320, 105);
        
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Net Trading P&L:', 50, 180);
        const netPnLColor = statement.trading.summary.netTradingPnLUSD >= 0 ? '#10B981' : '#EF4444';
        doc.fontSize(16).font('Helvetica-Bold').fillColor(netPnLColor);
        doc.text(`${statement.trading.summary.netTradingPnLUSD >= 0 ? '+' : ''}$${Math.abs(statement.trading.summary.netTradingPnLUSD).toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 200, 178);
      }
      
      // =============================================
      // REFERRAL EARNINGS
      // =============================================
      if (statement.referrals && statement.referrals.commissionsEarned.length > 0) {
        doc.addPage();
        
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#0B0E11');
        doc.text('Referral & Commission Earnings', 50, 50);
        
        doc.rect(50, 70, 500, 60).fill('#F0FDF4').stroke('#D1FAE5');
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#065F46');
        doc.text(`Total Referral Earnings: $${statement.referrals.summary.totalReferralEarningsUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 60, 90);
        
        let tableY = 160;
        doc.fillColor('#6B7280').fontSize(8);
        doc.text('Date', 50, tableY);
        doc.text('From User', 150, tableY);
        doc.text('Amount', 300, tableY);
        doc.text('Level', 400, tableY);
        
        tableY += 5;
        doc.moveTo(50, tableY).lineTo(550, tableY).stroke();
        
        for (const commission of statement.referrals.commissionsEarned.slice(0, 15)) {
          tableY += 20;
          doc.fillColor('#333333').fontSize(9);
          doc.text(new Date(commission.date).toLocaleDateString(), 50, tableY);
          doc.text(commission.fromUser?.name || 'Unknown', 150, tableY);
          doc.fillColor('#10B981');
          doc.text(`+$${commission.amountUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, 300, tableY);
          doc.fillColor('#6B7280');
          doc.text(`Level ${commission.level}`, 400, tableY);
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        doc.text(
          `₿itHash Capital - ${statement.statementType.toUpperCase()} Financial Statement | Page ${i + 1} of ${pageCount}`,
          50,
          doc.page.height - 40,
          { align: 'center' }
        );
        ...
        doc.text(
          `Generated: ${new Date().toLocaleString()} | Reference: ${statement.reference}`,
          50,
          doc.page.height - 30,
          { align: 'center' }
        );
        ...
        // Disclaimer
        doc.text(
          'This is an automated statement. For any discrepancies, please contact support@bithash.com',
          50,
          doc.page.height - 20,
...
// =============================================
// UPDATE: sendStatementEmail with Professional HTML (Maintains Your Brand)
// =============================================
async function sendStatementEmail(user, statement, pdfBuffer, chartData) {
  try {
    const periodText = statement.statementType === 'weekly' ? 'Weekly' : 'Monthly';
    const startDateStr = new Date(statement.period.startDate).toLocaleDateString('en-US', {
      year: 'numeric', month: 'long', day: 'numeric'
    });
    const endDateStr = new Date(statement.period.endDate).toLocaleDateString('en-US', {
      year: 'numeric', month: 'long', day: 'numeric'
    });
    
    const hasProfit = statement.summary.netProfitUSD >= 0;
    const profitLossSign = hasProfit ? '+' : '';
    const profitLossColor = hasProfit ? '#10B981' : '#EF4444';
    
    // Format numbers
    const formatUSD = (num) => `$${Math.abs(num).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    const formatPercent = (num) => `${num >= 0 ? '+' : ''}${num.toFixed(2)}%`;
    
    // Generate HTML email with your brand identity
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <title>${periodText} Financial Statement - ₿itHash Capital</title>
        <style>
          body {
        ...
        <div style="padding: 20px;">
          <div class="container">
            <div class="header">
              ...
              <p>Institutional Bitcoin Mining • Where Your Financial Goals Become Reality</p>
            </div>
            ...
            <div class="content">
              ...
              <div class="period-badge">
                ${periodText} Statement • ${startDateStr} - ${endDateStr}
              </div>
              
              <p>Your ${periodText.toLowerCase()} financial statement is ready. Here's a summary of your account performance:</p>
              ...
              <div class="summary-card">
                ...
                <div class="summary-grid">
                  <div class="summary-item">
                    ...
                    <div class="summary-value">${formatUSD(statement.openingBalances.totalUSD)}</div>
                  </div>
                  <div class="summary-item">
                    <div class="summary-label">Closing Balance</div>
                    <div class="summary-value">${formatUSD(statement.closingBalances.totalUSD)}</div>
                  </div>
                  <div class="summary-item">
                    <div class="summary-label">Net Change</div>
                    <div class="summary-value ${statement.netChangeUSD >= 0 ? 'profit-value' : 'loss-value'}">
                      ${statement.netChangeUSD >= 0 ? '+' : ''}${formatUSD(statement.netChangeUSD)}
                    </div>
                  </div>
                  <div class="summary-item">
                    <div class="summary-label">Net Profit/Loss</div>
                    <div class="summary-value ${hasProfit ? 'profit-value' : 'loss-value'}">
                      ${profitLossSign}${formatUSD(statement.summary.netProfitUSD)}
                      <span style="font-size: 12px;">(${formatPercent(statement.summary.roiPercentage)})</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <!-- Wallet Breakdown -->
              <div class="summary-title">💰 WALLET BREAKDOWN</div>
              <div class="wallet-grid">
                <div class="wallet-card">
                  <div class="wallet-label">Main Wallet</div>
                  <div class="wallet-value">${formatUSD(statement.closingBalances.mainWalletUSD)}</div>
                  <div style="font-size: 10px; color: #6B7280; margin-top: 5px;">Fluctuates with market</div>
                </div>
                <div class="wallet-card">
                  <div class="wallet-label">Active Mining</div>
                  <div class="wallet-value">${formatUSD(statement.closingBalances.activeWalletUSD)}</div>
                  <div style="font-size: 10px; color: #6B7280; margin-top: 5px;">Fixed value contracts</div>
                </div>
                <div class="wallet-card">
                  <div class="wallet-label">Matured Wallet</div>
                  <div class="wallet-value">${formatUSD(statement.closingBalances.maturedWalletUSD)}</div>
                  <div style="font-size: 10px; color: #6B7280; margin-top: 5px;">Completed returns</div>
                </div>
              </div>
              ...
              <div class="metric-row">
                ...
                <span class="metric-value profit-value">+${formatUSD(statement.transactions.summary.totalDepositsUSD)}</span>
              </div>
              <div class="metric-row">
                <span class="metric-label">Total Withdrawals</span>
                <span class="metric-value loss-value">-${formatUSD(statement.transactions.summary.totalWithdrawalsUSD)}</span>
              </div>
              <div class="metric-row">
                <span class="metric-label">Investment Returns</span>
                <span class="metric-value ${statement.investments.summary.totalReturnsEarnedUSD >= 0 ? 'profit-value' : 'loss-value'}">
                  ${statement.investments.summary.totalReturnsEarnedUSD >= 0 ? '+' : ''}${formatUSD(statement.investments.summary.totalReturnsEarnedUSD)}
                </span>
              </div>
              <div class="metric-row">
                <span class="metric-label">Trading P&L</span>
                <span class="metric-value ${statement.trading.summary.netTradingPnLUSD >= 0 ? 'profit-value' : 'loss-value'}">
                  ${statement.trading.summary.netTradingPnLUSD >= 0 ? '+' : ''}${formatUSD(statement.trading.summary.netTradingPnLUSD)}
                </span>
              </div>
              <div class="metric-row">
                <span class="metric-label">Referral Earnings</span>
                <span class="metric-value profit-value">+${formatUSD(statement.referrals.summary.totalReferralEarningsUSD)}</span>
              </div>
              <div class="metric-row">
                <span class="metric-label">Fees Paid</span>
                <span class="metric-value loss-value">-${formatUSD(statement.fees.summary.totalFeesUSD)}</span>
              </div>
              
              <!-- Top Crypto Holdings -->
              ${statement.closingBalances.cryptoDetails && statement.closingBalances.cryptoDetails.length > 0 ? `
              <div class="summary-title" style="margin-top: 24px;">🪙 TOP CRYPTO HOLDINGS</div>
              ${statement.closingBalances.cryptoDetails.slice(0, 5).map(crypto => `
                <div class="crypto-item">
                  <div>
                    <span class="crypto-name">${crypto.asset.toUpperCase()}</span>
                    <span class="crypto-amount">(${crypto.amount.toFixed(8)} ${crypto.asset.toUpperCase()})</span>
                  </div>
                  <div class="crypto-value">${formatUSD(crypto.usdValue)}</div>
                </div>
              `).join('')}
              ${statement.closingBalances.cryptoDetails.length > 5 ? `<div class="crypto-item"><span class="crypto-name">...and ${statement.closingBalances.cryptoDetails.length - 5} more assets</span></div>` : ''}
              ` : ''}
              
              <hr>
              
              <p style="text-align: center; font-size: 13px; color: #6B7280;">
                📎 A detailed PDF version of this statement with full transaction history and charts is attached to this email.
              </p>
              
              <div style="text-align: center;">
                <a href="https://www.bithashcapital.live/dashboard" class="button">📊 View Full Dashboard</a>
              </div>
              
              <hr>
              
              <p style="font-size: 11px; color: #6B7280; text-align: center;">
                This is an automated statement from ₿itHash Capital.<br>
                If you have any questions about your statement, please contact our support team at <a href="mailto:support@bithash.com" style="color: #F7A600;">support@bithash.com</a>
              </p>
            </div>
            ...
            <div class="footer">
              ...
              <p>
                <a href="mailto:support@bithash.com">support@bithash.com</a> | 
    ...
    const mailOptions = {
      ...
      to: user.email,
      subject: `📊 ${periodText} Financial Statement - ₿itHash Capital (${startDateStr})`,
      html: html,
      attachments: [
        {
          filename: `Financial_Statement_${user.firstName}_${user.lastName}_${startDateStr.replace(/\//g, '-')}.pdf`,
          content: pdfBuffer,
          contentType: 'application/pdf'
        }
      ]
...
// =============================================
// UPDATE: generateFinancialStatement to include chart data
// =============================================
async function generateFinancialStatementWithCharts(userId, startDate, endDate, statementType) {
  try {
    const statement = await generateFinancialStatement(userId, startDate, endDate, statementType);
    
    if (!statement) return null;
    
    // Generate chart data for PDF
    const dailyVolumeData = await getDailyVolumeData(userId, 7);
    
    const chartData = {
      dailyVolume: dailyVolumeData
    };
    
    return { statement, chartData };
    
  } catch (err) {
    console.error('Error generating statement with charts:', err);
    return null;
  }
}

// =============================================
// Helper: Get daily volume data for charts
// =============================================
async function getDailyVolumeData(userId, days = 7) {
...
// =============================================
// UPDATED: POST /api/admin/statements/generate endpoint
// =============================================
app.post('/api/admin/statements/generate', adminProtect, async (req, res) => {
  try {
    ...
    for (const user of usersToProcess) {
      try {
        const result = await generateFinancialStatementWithCharts(user._id, startDate, endDate, statementType);
        
        if (result) {
          const { statement, chartData } = result;
          
          // Generate production PDF with charts
          const pdfBuffer = await generateProfessionalPDFStatement(statement, user, chartData);
          
          // Send email with PDF attachment
          await sendStatementEmail(user, statement, pdfBuffer, chartData);
          ...
          results.failed++;
          results.errors.push(`Failed to generate statement for ${user.email}`);
        }
        ...
        await SystemLog.create({
          action: 'financial_statement_generation_failed',
          entity: 'financial_statement',
          performedBy: req.admin._id,
          performedByModel: 'Admin',
          performedByEmail: req.admin.email,
    ...
    res.status(200).json({
      status: 'success',
      message: batch ? `Processed ${results.processed} users, ${results.failed} failed` : `Statement generated and sent to ${usersToProcess[0]?.email}`,
      data: results
    });
