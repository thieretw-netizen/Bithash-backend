...
// =============================================
// EMAIL TRACKING SCHEMA - For marketing email delivery and read receipts
// =============================================
...
EmailTrackingSchema.index({ batchId: 1, createdAt: -1 });
EmailTrackingSchema.index({ status: 1, createdAt: -1 });
EmailTrackingSchema.index({ readAt: 1 });

const EmailTracking = mongoose.model('EmailTracking', EmailTrackingSchema);

// =============================================
// EMAIL TEMPLATE SCHEMA - For saved marketing templates
// =============================================
...
// =============================================
// EMAIL BATCH SCHEMA - For bulk email campaigns
// =============================================
...
const EmailBatchSchema = new mongoose.Schema({
  ...
  recipientType: {
    type: String,
    enum: ['all', 'specific', 'batch_file', 'external'],
    required: true
  },
  recipientList: [{
    email: String,
...
EmailBatchSchema.index({ status: 1, scheduledFor: 1 });
EmailBatchSchema.index({ sentBy: 1, createdAt: -1 });
      ...
      <div style="padding: 30px; background: #FFFFFF;">
        ...
        <div style="background: #F5F5F5; padding: 20px; border-radius: 12px; margin: 20px 0;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr style="border-top: 1px solid #E2E8F0;">
              <td style="padding: 8px 0;"><strong>Wallet Credited:</strong></tr>
...
case 'login_success':
  ...
  return isNaN(parsed.getTime()) ? new Date().toLocaleString() : parsed.toLocaleString();
      ...
      // =============================================
      // SISTER EMAIL TEMPLATE FOR EMAIL MARKETING SECTION
      // Includes actual pixel tracking for delivery and read receipts
      // =============================================
      case 'marketing_email':
        subject = data.subject || 'Important Update from ₿itHash Capital';
        
        // Generate unique tracking ID for this email
        const trackingId = `TRK-${Date.now()}-${Math.random().toString(36).substr(2, 10)}`;
        
        // Pixel tracking URL (actual pixel - not simulated)
        const trackingPixelUrl = `${process.env.API_URL || 'https://bithash-backend-kg7j.onrender.com'}/api/admin/email/track/${trackingId}`;
        
        // Device and location tracking
        const userAgent = req?.headers?.['user-agent'] || 'Unknown';
        const ipAddress = getRealClientIP(req) || 'Unknown';
        
        // HTML with pixel tracking (actual pixel)
        html = `
          <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #FFFFFF;">
            ${brandHeader}
            <div style="padding: 30px; background: #FFFFFF;">
              ...
              <!-- Footer info with tracking pixel -->
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #E5E7EB; font-size: 12px; color: #6C7480; text-align: center;">
                <p>₿itHash Capital · Where Your Financial Goals Become Reality</p>
                <p>This email was sent to ${email} on ${formattedTimestamp}</p>
                <p>
                  <a href="https://www.bithashcapital.live/unsubscribe" style="color: #F7A600; text-decoration: none;">Unsubscribe</a> |
                  <a href="https://www.bithashcapital.live/privacy" style="color: #F7A600; text-decoration: none;">Privacy Policy</a>
                </p>
              </div>
              
              <!-- ACTUAL TRACKING PIXEL - NOT SIMULATED -->
        ...
        // Store tracking info in database for delivery/read tracking
        if (data.trackingId) {
          await redis.setex(
            `email_tracking:${trackingId}`,
      ...
      // =============================================
      // BULK MARKETING EMAIL (Sister template - for mass sending)
      // =============================================
      case 'bulk_marketing':
        subject = data.subject || 'Important Update from ₿itHash Capital';
        
        // Generate unique tracking ID
        const bulkTrackingId = `BLK-${Date.now()}-${Math.random().toString(36).substr(2, 10)}`;
        
        // Pixel tracking URL
        const bulkTrackingPixelUrl = `${process.env.API_URL || 'https://bithash-backend-kg7j.onrender.com'}/api/admin/email/track/${bulkTrackingId}`;
        ...
        html = `
          <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #FFFFFF;">
            ...
            <div style="padding: 30px; background: #FFFFFF;">
              ...
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #E5E7EB; font-size: 12px; color: #6C7480; text-align: center;">
                <p>₿itHash Capital · Where Your Financial Goals Become Reality</p>
                <p>This bulk email was sent on ${formattedTimestamp}</p>
                <p>
                  <a href="https://www.bithashcapital.live/unsubscribe" style="color: #F7A600; text-decoration: none;">Manage Preferences</a>
                </p>
              </div>
              <img src="${bulkTrackingPixelUrl}" width="1" height="1" style="display:none;" alt="" />
            </div>
            ${brandFooter}
          </div>
        `;
        ...
        if (data.batchId) {
          await redis.sadd(`email_batch:${data.batchId}`, trackingId);
          await redis.expire(`email_batch:${data.batchId}`, 86400 * 30);
        }
        break;
...
// =============================================
// EMAIL MARKETING ENDPOINTS - For Admin HTML email marketing section
// =============================================

// GET /api/admin/email-history - Get email history with pagination
app.get('/api/admin/email-history', adminProtect, async (req, res) => {
  try {
    ...
    // Get email history from Redis or database
    let emails = [];
    let total = 0;

    try {
      // Get email history keys from Redis
      const emailKeys = await redis.keys('email_tracking:*');
      total = emailKeys.length;
      
      // Paginate
      const paginatedKeys = emailKeys.slice(skip, skip + limit);
      ...
      for (const key of paginatedKeys) {
        ...
        if (emailData) {
          ...
          emails.push({
            _id: key.replace('email_tracking:', ''),
            subject: parsed.subject,
            recipientCount: 1,
            deliveredCount: parsed.delivered ? 1 : 0,
            readCount: parsed.readCount || 0,
            openRate: parsed.readCount ? '100%' : '0%',
            status: parsed.delivered ? 'sent' : 'pending',
            createdAt: new Date(parsed.sentAt),
            delivered: parsed.delivered,
            recipients: [{
              email: parsed.email,
              delivered: parsed.delivered,
              readAt: parsed.readAt,
              status: parsed.readAt ? 'read' : (parsed.delivered ? 'delivered' : 'pending')
            }]
          });
        }
      }
    } catch (err) {
      console.error('Error fetching email history from Redis:', err);
    }

    const stats = {
      total: total,
      delivered: emails.filter(e => e.deliveredCount > 0).length,
      read: emails.filter(e => e.readCount > 0).length,
      openRate: total > 0 ? `${Math.round((emails.filter(e => e.readCount > 0).length / total) * 100)}%` : '0%'
    };
...
// GET /api/admin/email-history/:id - Get specific email details with read receipts
app.get('/api/admin/email-history/:id', adminProtect, async (req, res) => {
  try {
    const { id } = req.params;
    
    const emailData = await redis.get(`email_tracking:${id}`);
...
// GET /api/admin/email-templates - Get saved email templates
app.get('/api/admin/email-templates', adminProtect, async (req, res) => {
  try {
    ...
    try {
      ...
      const paginatedKeys = templateKeys.slice(skip, skip + limit);
      
      for (const key of paginatedKeys) {
        const templateData = await redis.get(key);
        if (templateData) {
          const parsed = JSON.parse(templateData);
          templates.push({
            _id: key.replace('email_template:', ''),
            name: parsed.name,
            subject: parsed.subject,
      ...
      console.error('Error fetching templates from Redis:', err);
      ...
      templates = [
        {
          ...
          name: 'Welcome Email',
          subject: 'Welcome to ₿itHash Capital!',
          content: '<h2>Welcome to our platform!</h2><p>We are excited to have you on board.</p>',
          type: 'html',
          createdAt: new Date(),
          updatedAt: new Date()
        },
        {
          ...
          content: '<h2>Investment Confirmed</h2><p>Your investment has been successfully processed.</p>',
          type: 'html',
          createdAt: new Date(),
          updatedAt: new Date()
        }
...
app.get('/api/admin/email-templates/:id', adminProtect, async (req, res) => {
  try {
    ...
    const templateData = await redis.get(`email_template:${id}`);
...
app.post('/api/admin/email-templates', adminProtect, async (req, res) => {
  try {
    ...
    await redis.set(`email_template:${templateId}`, JSON.stringify(templateData));
    await redis.expire(`email_template:${templateId}`, 86400 * 90); // 90 days
...
app.delete('/api/admin/email-templates/:id', adminProtect, async (req, res) => {
  try {
    ...
    const exists = await redis.del(`email_template:${id}`);
...
// POST /api/admin/send-email - Send marketing email (handles all recipient types)
  ...
  try {
    ...
    if (recipients && recipients.length > 0) {
      if (recipients[0] === 'all') {
        ...
        recipientEmails = users.map(u => ({
          email: u.email,
          name: `${u.firstName} ${u.lastName}`,
          userId: u._id
        }));
      } else {
        // Specific recipients or external emails
        for (const recipient of recipients) {
          if (recipient.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
            // External email
            recipientEmails.push({ email: recipient, name: 'User' });
          } else {
            // User ID
            const user = await User.findById(recipient).select('email firstName lastName');
            if (user) {
    ...
    // Generate batch ID for bulk tracking
    const batchId = `BATCH_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
    
    // Send emails to all recipients
    for (const recipient of recipientEmails) {
      try {
        // Generate tracking ID for this specific email
        const trackingId = `TRK_${Date.now()}_${Math.random().toString(36).substr(2, 10)}`;
        
        // Send email using the sister template with tracking
        await sendAutomatedEmail(
          { 
            email: recipient.email, 
            firstName: recipient.name.split(' ')[0] || 'Valued Customer' 
          },
          'marketing_email',
          {
            subject: subject,
...
// GET /api/admin/email/track/:trackingId - Tracking pixel endpoint (actual pixel)
app.get('/api/admin/email/track/:trackingId', async (req, res) => {
  try {
    const { trackingId } = req.params;
    
    // Get tracking info
    const trackingData = await redis.get(`email_tracking:${trackingId}`);
    
    if (trackingData) {
      const parsed = JSON.parse(trackingData);
      
      // Update read count and read time
      parsed.readCount = (parsed.readCount || 0) + 1;
      if (!parsed.readAt) {
        parsed.readAt = new Date();
      }
      
      // Update location info from request
      const deviceInfo = await getUserDeviceInfo(req);
      parsed.readLocation = deviceInfo.location;
      parsed.readIp = deviceInfo.ip;
      parsed.readUserAgent = req.headers['user-agent'];
      
      // Save updated tracking info
      await redis.setex(`email_tracking:${trackingId}`, 86400 * 30, JSON.stringify(parsed));
    }
