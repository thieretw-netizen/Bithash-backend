  ...
  priceUpdateInterval = setInterval(async () => {
    try {
      ...
      const pricePromises = assets.map(async (asset) => {
        ...
        if (price && price > 0) {
          const previousPrice = lastBroadcastedPrices[asset.toLowerCase()]?.price || price;
      ...
      if (Object.keys(priceUpdates).length > 0 && io) {
        ...
        for (const [asset, data] of Object.entries(priceUpdates)) {
          try {
            const response = await axios.get(
              `https://api.coingecko.com/api/v3/simple/price?ids=${mapSymbolToCoinGeckoId(asset)}&vs_currencies=usd&include_24hr_change=true`,
              { timeout: 3000 }
            );
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        let previousMainValue = currentMainValue;
        let previousMaturedValue = currentMaturedValue;
        
        if (lastDate === today) {
          const cachedPrev = await redis.get(previousDayKey);
          if (cachedPrev) {
            const prev = JSON.parse(cachedPrev);
            previousMainValue = prev.main || currentMainValue;
        ...
        console.error(`Error calculating PnL for user ${user._id}:`, userErr.message);
      }
    }
    
  } catch (err) {
    console.error('Error in PnL calculation:', err);
  } finally {
    pnlCalculationLock = false;
  }
};
...
const recalculateAllWalletValuesRealtime = async (io, currentPrices) => {
  ...
  try {
    ...
    console.log(`💰 Real-time wallet values recalculated for ${users.length} users`);
...
app.get('/api/users/assets', protect, async (req, res) => {
  try {
    ...
    if (user.balances.main && user.balances.main instanceof Map) {
      for (const [asset, balance] of user.balances.main.entries()) {
        if (balance > 0 && asset !== 'usd') {
          if (!allAssetsMap.has(asset)) {
            allAssetsMap.set(asset, { totalBalance: 0, mainBalance: 0, maturedBalance: 0 });
          }
          const entry = allAssetsMap.get(asset);
          entry.totalBalance += balance;
          entry.mainBalance += balance;
        }
      }
    }
    ...
    if (user.balances.matured && user.balances.matured instanceof Map) {
      for (const [asset, balance] of user.balances.matured.entries()) {
        if (balance > 0 && asset !== 'usd') {
          ...
          const entry = allAssetsMap.get(asset);
          entry.totalBalance += balance;
          entry.maturedBalance += balance;
        }
      }
    }
    
    const assetsList = Array.from(allAssetsMap.keys());
    ...
    if (failedAssets.length > 0) {
      return res.status(503).json({
        status: 'error',
        message: `Unable to fetch current prices for: ${failedAssets.join(', ').toUpperCase()}. Please try again later.`,
        missingPrices: failedAssets,
        retryAfter: 10
      });
    }
    ...
    for (const [asset, balances] of allAssetsMap.entries()) {
      ...
      const change24h = changeMap.get(asset) || 0;
    ...
    assetData.forEach(a => {
      console.log(`   ${a.symbol.toUpperCase()}: ${a.balance} @ $${a.currentPrice} = $${a.currentValue.toFixed(2)} (Main: ${a.mainBalance}, Matured: ${a.maturedBalance})`);
    });
...
app.get('/api/users/pnl', protect, async (req, res) => {
  try {
    ...
    if (user.balances && user.balances.main) {
      for (const [asset, balance] of user.balances.main.entries()) {
        if (balance > 0 && asset !== 'usd') {
          ...
          let change24h = 0;
          try {
            const priceData = await getCryptoPriceWithChange(asset.toUpperCase());
          ...
          assetChanges.push({
            ...
            currentPrice: currentPrice,
            change24h: change24h,
            currentValue: balance * (currentPrice || 0)
          });
        }
      }
    }
    ...
    res.status(200).json({
      ...
      data: {
        main: {
          currentValue: currentMainValue,
          previousValue: previousMainValue,
          pnl: mainPnL,
          pnlPercentage: mainPnLPercent,
          isPositive: mainPnL >= 0
        },
        matured: {
          currentValue: currentMaturedValue,
          previousValue: previousMaturedValue,
          pnl: maturedPnL,
          pnlPercentage: maturedPnLPercent,
          isPositive: maturedPnL >= 0
        },
